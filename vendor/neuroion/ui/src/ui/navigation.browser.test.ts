...
const originalConnect = NeuroionApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("neuroion-app") as NeuroionApp;
...
beforeEach(() => {
  NeuroionApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__NEUROION_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  NeuroionApp.prototype.connect = originalConnect;
  window.__NEUROION_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/neuroion/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/neuroion");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/neuroion/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__NEUROION_CONTROL_UI_BASE_PATH__ = "/neuroion";
    const app = mountApp("/neuroion/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/neuroion");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/neuroion/sessions");
  });
