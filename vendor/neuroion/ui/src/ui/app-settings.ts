import type { NeuroionApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as NeuroionApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as NeuroionApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as NeuroionApp);
  }
  if (host.tab === "agents") {
    const app = host as unknown as NeuroionApp;
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as NeuroionApp);
    await loadDevices(host as unknown as NeuroionApp);
    await loadConfig(host as unknown as NeuroionApp);
    await loadExecApprovals(host as unknown as NeuroionApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as NeuroionApp);
    await loadConfig(host as unknown as NeuroionApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as NeuroionApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as NeuroionApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__NEUROION_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as NeuroionApp, false),
    loadPresence(host as unknown as NeuroionApp),
    loadSessions(host as unknown as NeuroionApp),
    loadCronStatus(host as unknown as NeuroionApp),
    loadDebug(host as unknown as NeuroionApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as NeuroionApp, true),
    loadConfigSchema(host as unknown as NeuroionApp),
    loadConfig(host as unknown as NeuroionApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as NeuroionApp, false),
    loadCronStatus(host as unknown as NeuroionApp),
    loadCronJobs(host as unknown as NeuroionApp),
  ]);
}
