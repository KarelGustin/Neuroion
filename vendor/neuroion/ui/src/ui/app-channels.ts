...
export async function handleWhatsAppStart(host: NeuroionApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: NeuroionApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: NeuroionApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: NeuroionApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: NeuroionApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: NeuroionApp): string {
...
export function handleNostrProfileCancel(host: NeuroionApp) {
...
export function handleNostrProfileToggleAdvanced(host: NeuroionApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: NeuroionApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: NeuroionApp) {
  const state = host.nostrProfileFormState;
