...
metadata:
  { "neuroion": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
```bash
SOCKET_DIR="${NEUROION_TMUX_SOCKET_DIR:-${NEUROIONBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/neuroion-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/neuroion.sock"
SESSION=neuroion-python
...
- Use `NEUROION_TMUX_SOCKET_DIR` (legacy `NEUROIONBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$NEUROION_TMUX_SOCKET_DIR/neuroion.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `NEUROION_TMUX_SOCKET_DIR`).
