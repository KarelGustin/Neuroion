---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Neuroion deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Neuroion cron scheduling for periodic checks, or version status checks on a machine running Neuroion (laptop, workstation, Pi, VPS).
---

# Neuroion Host Hardening

## Overview

Assess and harden the host running Neuroion, then align it to a user-defined risk tolerance without breaking access. Use Neuroion security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Neuroion changes the host firewall, SSH, or OS updates; it does not.
...
5. Neuroion gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Neuroion can access sensitive data.
...
### 2) Run Neuroion security audits (read-only)

As part of the default read-only checks, run `neuroion security audit --deep`. Only offer alternatives if the user requests them:

1. `neuroion security audit` (faster, non-probing)
2. `neuroion security audit --json` (structured output)

Offer to apply Neuroion safe defaults (numbered):

1. `neuroion security audit --fix`

Be explicit that `--fix` only tightens Neuroion defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Neuroion version/update status (read-only)

As part of the default read-only checks, run `neuroion update status`.
...
- Credential hygiene notes (location of Neuroion creds, prefer disk encryption)
...
- Neuroion security audit (re-run)
...
After Neuroion install or first hardening pass, run at least one baseline audit and version check:

- `neuroion security audit`
- `neuroion security audit --deep`
- `neuroion update status`

Ongoing monitoring is recommended. Use the Neuroion cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `neuroion cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `neuroion update status`
...
Before creating, `neuroion cron list` and match on exact `name`. If found, `neuroion cron edit <id> ...`.
If not found, `neuroion cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `neuroion update status` (preferred for source checkouts and channels)
2. `npm view neuroion version` (published npm version)

## Neuroion command accuracy

Use only supported commands and flags:

- `neuroion security audit [--deep] [--fix] [--json]`
- `neuroion status` / `neuroion status --deep`
- `neuroion health --json`
- `neuroion update status`
- `neuroion cron add|list|runs|run`

Do not invent CLI flags or imply Neuroion enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Neuroion compaction:
