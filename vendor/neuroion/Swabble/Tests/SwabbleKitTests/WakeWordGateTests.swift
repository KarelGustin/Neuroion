...
@Suite struct WakeWordGateTests {
    @Test func matchRequiresGapAfterTrigger() {
        let transcript = "hey neuroion do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["neuroion"], minPostTriggerGap: 0.3)
    ...
    @Test func matchHandlesMultiWordTriggers() {
        let transcript = "hey neuroion do it"
        let segments = makeSegments(
            transcript: transcript,
            words: [
                ("hey", 0.0, 0.1),
        ...
        let config = WakeWordGateConfig(triggers: ["hey neuroion"], minPostTriggerGap: 0.3)
        let match = WakeWordGate.match(transcript: transcript, segments: segments, config: config)
