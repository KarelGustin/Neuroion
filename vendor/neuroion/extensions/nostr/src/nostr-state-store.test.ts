import type { PluginRuntime } from "neuroion/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.NEUROION_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "neuroion-nostr-"));
  process.env.NEUROION_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.NEUROION_STATE_DIR?.trim() || env.NEUROION_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".neuroion");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.NEUROION_STATE_DIR = previous;
    }
