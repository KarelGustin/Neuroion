...
function resolveNeuroionRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.NEUROION_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set NEUROION_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveNeuroionRoot(), "dist", "extensionAPI.js");
