...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as NeuroionConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as NeuroionConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as NeuroionConfig);
