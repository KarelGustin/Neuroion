...
metadata: { "neuroion": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Neuroion `sessions_spawn`
- **File I/O** == Neuroion `read`/`write`
- **Remote fetch** == Neuroion `web_fetch` (or `exec` with curl when POST is required)
