...
`neuroion.invoke --each`) without adding new Neuroion code per workflow.
...
This extension depends on Neuroion internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Neuroion extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.neuroion/extensions` as a standalone plugin directory.
