...
// NOTE: This extension is intended to be bundled with Neuroion.
// When running from source (tests/dev), Neuroion internals live under src/.
...
export function createLlmTaskTool(api: NeuroionPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via neuroion.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "neuroion-llm-task-"));
