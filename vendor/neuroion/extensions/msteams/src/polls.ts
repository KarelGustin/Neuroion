...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["neuroionPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["neuroion", "pollId"]) ??
    readNestedString(value, ["neuroion", "poll", "id"]) ??
    readNestedString(value, ["data", "neuroionPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "neuroion", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          neuroionPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "neuroion poll vote",
          displayText: "Vote recorded",
          value: { neuroionPollId: pollId, pollId },
        },
      },
    ],
  };
