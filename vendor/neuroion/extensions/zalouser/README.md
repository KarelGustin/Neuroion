...
Neuroion extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
neuroion channels login --channel zalouser
...
```bash
neuroion message send --channel zalouser --target <threadId> --message "Hello from Neuroion!"
```
...
```bash
neuroion channels login --channel zalouser              # Login via QR
neuroion channels login --channel zalouser --account work
neuroion channels status --probe
neuroion channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
neuroion directory self --channel zalouser
neuroion directory peers list --channel zalouser --query "name"
neuroion directory groups list --channel zalouser --query "work"
neuroion directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
neuroion message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
neuroion message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `neuroion channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
neuroion channels login --channel zalouser --account work
neuroion message send --channel zalouser --account work --target <id> --message "Hello"
