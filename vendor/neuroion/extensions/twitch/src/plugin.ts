/**
 * Twitch channel plugin for Neuroion.
 *
...
import { buildChannelConfigSchema } from "neuroion/plugin-sdk";
import type {
 ...
 * for Neuroion. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: NeuroionConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: NeuroionConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: NeuroionConfig): boolean => {
