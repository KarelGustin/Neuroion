...
# Neuroion Auth Widget for Termux
...
SERVER="${NEUROION_SERVER:-${NEUROIONBOT_SERVER:-l36}}"

# Check auth status
termux-toast "Checking Neuroion auth..."

STATUS=$(ssh "$SERVER" '$HOME/neuroion/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/neuroion/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|NEUROION_EXPIRING|NEUROIONBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/neuroion/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|NEUROION_EXPIRED|NEUROIONBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Neuroion won't work until re-auth"
                ;;
        esac
        ;;
