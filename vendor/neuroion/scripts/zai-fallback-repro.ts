...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "neuroion-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "neuroion.json");
  ...
  const sessionId =
    process.env.NEUROION_ZAI_FALLBACK_SESSION_ID ??
    process.env.NEUROIONBOT_ZAI_FALLBACK_SESSION_ID ??
    randomUUID();
  ...
  const baseEnv: NodeJS.ProcessEnv = {
    ...
    NEUROIONBOT_CONFIG_PATH: configPath,
    NEUROIONBOT_STATE_DIR: stateDir,
    ZAI_API_KEY: zaiKey,
    Z_AI_API_KEY: "",
  };
  ...
  const run1 = await runCommand(
    "run1",
    ["neuroion", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["neuroion", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
