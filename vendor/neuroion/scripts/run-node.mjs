...
const logRunner = (message) => {
  if (env.NEUROION_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[neuroion] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["neuroion.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
