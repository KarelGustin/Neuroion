#!/bin/bash
# Setup Neuroion Auth Management System
...
echo "=== Neuroion Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'neuroion-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Neuroion message: Send warning via Neuroion itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/neuroion-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/neuroion-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/neuroion-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now neuroion-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/NeuroionAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/NeuroionAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Neuroion*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (NeuroionAuth) shows status and opens auth URL if needed."
echo "The full widget (NeuroionAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status neuroion-auth-monitor.timer"
echo ""
echo "Quick commands:"
