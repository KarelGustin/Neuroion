...
IMAGE_NAME="neuroion-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="neuroion-net-e2e-$$"
GW_NAME="neuroion-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "NEUROION_GATEWAY_TOKEN=$TOKEN" \
	  -e "NEUROION_SKIP_CHANNELS=1" \
	  -e "NEUROION_SKIP_GMAIL_WATCHER=1" \
	  -e "NEUROION_SKIP_CRON=1" \
	  -e "NEUROION_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
