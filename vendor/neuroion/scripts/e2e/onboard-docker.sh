...
IMAGE_NAME="neuroion-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/neuroion-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/neuroion-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/neuroion-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/neuroion-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/neuroion-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/.neuroion/workspace"
    config_path="$HOME/.neuroion/neuroion.json"
    sessions_dir="$HOME/.neuroion/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.neuroion/neuroion.json" <<'"'"'JSON'"'"'
{
