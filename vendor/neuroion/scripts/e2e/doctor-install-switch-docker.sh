...
IMAGE_NAME="neuroion-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/neuroion-bin:$PATH"
  mkdir -p /tmp/neuroion-bin

  cat > /tmp/neuroion-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/neuroion-bin/systemctl

  cat > /tmp/neuroion-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/neuroion-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/neuroion"
  npm_entry="/tmp/npm-prefix/lib/node_modules/neuroion/dist/index.js"
  git_entry="/app/dist/index.js"
  git_cli="/app/neuroion.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/neuroion-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/neuroion-gateway.service"
    if [ ! -f "$unit_path" ]; then
