...
IMAGE_NAME="neuroion-plugins-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  home_dir=$(mktemp -d "/tmp/neuroion-plugins-e2e.XXXXXX")
  export HOME="$home_dir"
  mkdir -p "$HOME/.neuroion/extensions/demo-plugin"

  cat > "$HOME/.neuroion/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.neuroion/extensions/demo-plugin/neuroion.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/neuroion-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@neuroion/demo-plugin-tgz",
  "version": "0.0.1",
...
JS
  cat > "$pack_dir/package/neuroion.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/neuroion-plugin-dir.XXXXXX")"
...
JS
  cat > "$dir_plugin/neuroion.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/neuroion-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@neuroion/demo-plugin-file",
  "version": "0.0.1",
  "neuroion": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$file_pack_dir/package/neuroion.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
