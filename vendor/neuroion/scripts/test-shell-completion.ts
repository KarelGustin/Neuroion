 ...
 * `neuroion update`. Use it to verify the completion installation flow
...
\`neuroion update\`, \`neuroion doctor\`, and \`neuroion onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Neuroion Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`neuroion completion --install\` later to enable.`));
    return;
  }
