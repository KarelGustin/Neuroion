...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/neuroion/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Neuroion Re-Auth" -c "After getting key, run: ssh $SERVER '~/neuroion/scripts/mobile-reauth.sh'" --id neuroion-auth
        ;;
    *)
