...
ZIP=${1:?"Usage: $0 Neuroion-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/neuroion/neuroion/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Neuroion-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/neuroion/neuroion/releases/download/v${VERSION}/"}
