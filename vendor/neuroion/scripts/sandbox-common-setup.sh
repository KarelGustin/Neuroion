...
BASE_IMAGE="${BASE_IMAGE:-neuroion-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-neuroion-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=neuroion.sandbox=1)
NOTE
