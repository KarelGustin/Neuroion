...
INSTALL_URL="${NEUROION_INSTALL_URL:-https://neuroion.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${NEUROION_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${NEUROION_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="neuroion"
PACKAGE_NAME="${NEUROION_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (NEUROION_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${NEUROION_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${NEUROION_INSTALL_LATEST_OUT:-}"
fi
