...
#
# Suggested cron: */30 * * * * /home/admin/neuroion/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Neuroion notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., neuroion-alerts)
...
STATE_FILE="$HOME/.neuroion/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Neuroion if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use neuroion
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Neuroion to $NOTIFY_PHONE..."
            neuroion send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Neuroion is down. Run: ssh l36 '~/neuroion/scripts/mobile-reauth.sh'" "urgent"
    exit 1
