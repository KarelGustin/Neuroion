...
# Build and bundle Neuroion into a minimal .app we can open.
# Outputs to dist/Neuroion.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Neuroion.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Neuroion"
BUNDLE_ID="${BUNDLE_ID:-ai.neuroion.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/neuroion/neuroion/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Neuroion/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :NeuroionBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :NeuroionGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Neuroion"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Neuroion"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Neuroion"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Neuroion" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Neuroion/Resources/Neuroion.icns" "$APP_ROOT/Contents/Resources/Neuroion.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Neuroion/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying NeuroionKit resources"
NEUROIONKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/NeuroionKit_NeuroionKit.bundle"
if [ -d "$NEUROIONKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/NeuroionKit_NeuroionKit.bundle"
  cp -R "$NEUROIONKIT_BUNDLE" "$APP_ROOT/Contents/Resources/NeuroionKit_NeuroionKit.bundle"
else
  echo "WARN: NeuroionKit resource bundle not found at $NEUROIONKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Neuroion"
killall -q Neuroion 2>/dev/null || true
