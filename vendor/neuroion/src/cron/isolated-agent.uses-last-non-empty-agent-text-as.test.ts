...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "neuroion-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".neuroion", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<NeuroionConfig> = {},
): NeuroionConfig {
  const base: NeuroionConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "neuroion"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".neuroion", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
