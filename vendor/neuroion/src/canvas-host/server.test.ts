...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("neuroionCanvasA2UIAction");
    expect(out).toContain("neuroionSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "neuroion-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("neuroionSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.neuroionA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__neuroion__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("neuroion-a2ui-host");
      expect(html).toContain("neuroionCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__neuroion__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("neuroionA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
