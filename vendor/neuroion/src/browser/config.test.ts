...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const neuroion = resolveProfile(resolved, "neuroion");
    expect(neuroion?.driver).toBe("neuroion");
    expect(neuroion?.cdpPort).toBe(18800);
    expect(neuroion?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from NEUROION_GATEWAY_PORT when unset", () => {
    const prev = process.env.NEUROION_GATEWAY_PORT;
    process.env.NEUROION_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const neuroion = resolveProfile(resolved, "neuroion");
      expect(neuroion?.cdpPort).toBe(19012);
      expect(neuroion?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.NEUROION_GATEWAY_PORT;
      } else {
        process.env.NEUROION_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const neuroion = resolveProfile(resolved, "neuroion");
      expect(neuroion?.cdpPort).toBe(19022);
      expect(neuroion?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.NEUROION_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "neuroion");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        neuroion: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("neuroion");
  });
});
