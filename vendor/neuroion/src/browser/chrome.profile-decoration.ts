...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".neuroion-profile-decorated");
}
 ...
 */
export function decorateNeuroionProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_NEUROION_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_NEUROION_BROWSER_COLOR).toUpperCase();
