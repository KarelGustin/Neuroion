  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        const existing = document.querySelectorAll("[data-neuroion-labels]");
        existing.forEach((el) => el.remove());

        const root = document.createElement("div");
        root.setAttribute("data-neuroion-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-neuroion-labels", "1");
          ...
          tag.setAttribute("data-neuroion-labels", "1");
