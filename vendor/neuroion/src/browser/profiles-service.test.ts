...
vi.mock("./chrome.js", () => ({
  resolveNeuroionUserDataDir: vi.fn(() => "/tmp/neuroion-test/neuroion/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveNeuroionUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "neuroion-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveNeuroionUserDataDir).mockReturnValue(userDataDir);
