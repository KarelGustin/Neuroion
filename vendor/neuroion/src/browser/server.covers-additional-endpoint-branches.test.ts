  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "neuroion",
        profiles: {
          neuroion: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchNeuroionChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
    launchCalls.push({ port: profile.cdpPort });
    reachable = true;
    return {
      pid: 123,
      exe: { kind: "chrome", path: "/fake/chrome" },
      userDataDir: "/tmp/neuroion",
      cdpPort: profile.cdpPort,
      startedAt: Date.now(),
      proc,
    };
  }),
  resolveNeuroionUserDataDir: vi.fn(() => "/tmp/neuroion"),
  stopNeuroionChrome: vi.fn(async () => {
    reachable = false;
  }),
}));
...
describe("browser control server", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.NEUROION_GATEWAY_PORT;
    process.env.NEUROION_GATEWAY_PORT = String(testPort - 2);
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPort === undefined) {
      ...
      process.env.NEUROION_GATEWAY_PORT = prevGatewayPort;
    }
...
describe("backward compatibility (profile parameter)", () => {
  ...
  it("GET / without profile uses default profile", async () => {
    ...
    // Should use default profile (neuroion)
    expect(status.profile).toBe("neuroion");
  });
  ...
  it("POST /start without profile uses default profile", async () => {
    ...
    expect(result.profile).toBe("neuroion");
  });
  ...
  it("GET /profiles returns list of profiles", async () => {
    ...
    // Should at least have the default neuroion profile
    expect(result.profiles.some((p) => p.name === "neuroion")).toBe(true);
  });

  it("GET /tabs?profile=neuroion returns tabs for specified profile", async () => {
    const { startBrowserControlServerFromConfig } = await import("./server.js");
    await startBrowserControlServerFromConfig();
    const base = `http://127.0.0.1:${testPort}`;
    ...
    const result = (await realFetch(`${base}/tabs?profile=neuroion`).then((r) => r.json())) as {
      running: boolean;
      tabs: unknown[];
    };
    expect(result.running).toBe(true);
  ...
  it("POST /tabs/open?profile=neuroion opens tab in specified profile", async () => {
    const { startBrowserControlServerFromConfig } = await import("./server.js");
    await startBrowserControlServerFromConfig();
    const base = `http://127.0.0.1:${testPort}`;
    ...
    const result = (await realFetch(`${base}/tabs/open?profile=neuroion`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ url: "https://example.com" }),
    }).then((r) => r.json())) as { targetId?: string };
