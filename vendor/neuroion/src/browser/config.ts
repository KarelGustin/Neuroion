import type { BrowserConfig, BrowserProfileConfig, NeuroionConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_NEUROION_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_NEUROION_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "neuroion" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_NEUROION_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_NEUROION_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Neuroion browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for neuroion/neuroion CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: NeuroionConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_NEUROION_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "neuroion";
