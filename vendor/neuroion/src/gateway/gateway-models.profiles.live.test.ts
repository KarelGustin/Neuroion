...
import type { NeuroionConfig, ModelProviderConfig } from "../config/types.js";
import { resolveNeuroionAgentDir } from "../agents/agent-paths.js";
...
import { ensureNeuroionModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.NEUROION_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.NEUROION_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.NEUROION_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<NeuroionConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.NEUROION_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.NEUROION_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveNeuroionAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "neuroion-live-state-"));
  process.env.NEUROION_STATE_DIR = tempStateDir;
  ...
  process.env.NEUROION_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.neuroion-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveNeuroionAgentDir();
  const sanitizedCfg: NeuroionConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Neuroion live tool probe (local, safe): " +
    ...
    process.env.NEUROION_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.NEUROION_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureNeuroionModelsJson(cfg);
      ...
      const rawModels = process.env.NEUROION_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.neuroion-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
