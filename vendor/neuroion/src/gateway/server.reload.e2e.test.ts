...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.NEUROION_SKIP_CHANNELS;
    prevSkipGmail = process.env.NEUROION_SKIP_GMAIL_WATCHER;
    process.env.NEUROION_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.NEUROION_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.NEUROION_SKIP_GMAIL_WATCHER;
    } else {
      process.env.NEUROION_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
