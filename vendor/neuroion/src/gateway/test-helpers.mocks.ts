...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `neuroion-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.NEUROION_CONFIG_PATH = path.join(root, "neuroion.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "neuroion.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "neuroion-gateway-test"),
        ...fileDefaults,
