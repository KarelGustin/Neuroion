...
export function resolveGatewayAuth(params: {
  ...
  const token =
    authConfig.token ?? env.NEUROION_GATEWAY_TOKEN ?? env.NEUROIONBOT_GATEWAY_TOKEN ?? undefined;
  const password =
    authConfig.password ??
    env.NEUROION_GATEWAY_PASSWORD ??
    env.NEUROIONBOT_GATEWAY_PASSWORD ??
    undefined;
...
export function assertGatewayAuthConfigured(auth: ResolvedGatewayAuth): void {
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or NEUROION_GATEWAY_TOKEN)",
    );
  }
