...
import { ensureNeuroionCliOnPath } from "../infra/path-env.js";
...
export async function startGatewayServer(
  ...
  process.env.NEUROION_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "NEUROION_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "NEUROION_RAW_STREAM_PATH",
  ...
  if (configSnapshot.legacyIssues.length > 0) {
    ...
    if (!migrated) {
      throw new Error(
        `Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("neuroion doctor")}" to migrate.`,
      );
    }
  ...
  if (configSnapshot.exists && !configSnapshot.valid) {
    ...
    throw new Error(
      `Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("neuroion doctor")}" to repair, then retry.`,
    );
  }
