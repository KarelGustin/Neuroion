...
export function resolveHooksConfig(cfg: NeuroionConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-neuroion-token"] === "string"
      ? req.headers["x-neuroion-token"].trim()
      : "";
  if (headerToken) {
