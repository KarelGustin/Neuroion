...
describe("web monitor inbox", () => {
  beforeEach(() => {
    ...
    authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "neuroion-auth-"));
  });
  ...
  it("logs inbound bodies to file", async () => {
    const logPath = path.join(os.tmpdir(), `neuroion-log-test-${crypto.randomUUID()}.log`);
