...
describe("hasAnyWhatsAppAuth", () => {
  ...
  beforeEach(() => {
    previousOauthDir = process.env.NEUROION_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "neuroion-oauth-"));
    process.env.NEUROION_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterEach(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.NEUROION_OAUTH_DIR = previousOauthDir;
    }
    if (tempOauthDir) {
  ...
  it("includes authDir overrides", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "neuroion-wa-auth-"));
    try {
