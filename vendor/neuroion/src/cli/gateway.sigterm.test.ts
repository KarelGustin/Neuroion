...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "neuroion-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      NEUROION_NO_RESPAWN: "1",
      NEUROION_STATE_DIR: stateDir,
      NEUROION_SKIP_CHANNELS: "1",
      NEUROION_SKIP_GMAIL_WATCHER: "1",
      NEUROION_SKIP_CRON: "1",
      NEUROION_SKIP_BROWSER_CONTROL_SERVER: "1",
      NEUROION_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "neuroion-entry-bootstrap.mjs");
