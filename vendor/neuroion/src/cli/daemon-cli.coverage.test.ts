...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    NEUROION_STATE_DIR: process.env.NEUROION_STATE_DIR,
    NEUROION_CONFIG_PATH: process.env.NEUROION_CONFIG_PATH,
    NEUROION_GATEWAY_PORT: process.env.NEUROION_GATEWAY_PORT,
    NEUROION_PROFILE: process.env.NEUROION_PROFILE,
  };

  beforeEach(() => {
    process.env.NEUROION_STATE_DIR = "/tmp/neuroion-cli-state";
    process.env.NEUROION_CONFIG_PATH = "/tmp/neuroion-cli-state/neuroion.json";
    delete process.env.NEUROION_GATEWAY_PORT;
    delete process.env.NEUROION_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.NEUROION_STATE_DIR !== undefined) {
      process.env.NEUROION_STATE_DIR = originalEnv.NEUROION_STATE_DIR;
    } else {
      delete process.env.NEUROION_STATE_DIR;
    }

    if (originalEnv.NEUROION_CONFIG_PATH !== undefined) {
      process.env.NEUROION_CONFIG_PATH = originalEnv.NEUROION_CONFIG_PATH;
    } else {
      delete process.env.NEUROION_CONFIG_PATH;
    }

    if (originalEnv.NEUROION_GATEWAY_PORT !== undefined) {
      process.env.NEUROION_GATEWAY_PORT = originalEnv.NEUROION_GATEWAY_PORT;
    } else {
      delete process.env.NEUROION_GATEWAY_PORT;
    }

    if (originalEnv.NEUROION_PROFILE !== undefined) {
      process.env.NEUROION_PROFILE = originalEnv.NEUROION_PROFILE;
    } else {
      delete process.env.NEUROION_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        NEUROION_PROFILE: "dev",
        NEUROION_STATE_DIR: "/tmp/neuroion-daemon-state",
        NEUROION_CONFIG_PATH: "/tmp/neuroion-daemon-state/neuroion.json",
        NEUROION_GATEWAY_PORT: "19001",
      },
