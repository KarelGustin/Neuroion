...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Neuroion Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(neuroion completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# Neuroion Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(neuroion completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $NEUROION_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "neuroion") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c neuroion -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'neuroion gateway' and not 'neuroion other gateway', we need to check the sequence.
