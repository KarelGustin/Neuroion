...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("neuroion node install"), formatCliCommand("neuroion node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("neuroion node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    NEUROION_LOG_PREFIX: baseEnv.NEUROION_LOG_PREFIX ?? "node",
