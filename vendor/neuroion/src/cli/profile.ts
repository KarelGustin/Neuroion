...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.neuroion${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.NEUROION_PROFILE = profile;

  const stateDir = env.NEUROION_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.NEUROION_STATE_DIR?.trim()) {
    env.NEUROION_STATE_DIR = stateDir;
  }

  if (!env.NEUROION_CONFIG_PATH?.trim()) {
    env.NEUROION_CONFIG_PATH = path.join(stateDir, "neuroion.json");
  }

  if (profile === "dev" && !env.NEUROION_GATEWAY_PORT?.trim()) {
    env.NEUROION_GATEWAY_PORT = "19001";
  }
}
