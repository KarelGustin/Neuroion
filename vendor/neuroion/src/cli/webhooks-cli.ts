...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.neuroion.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Neuroion hooks")
    ...
    .option("--hook-url <url>", "Neuroion hook URL")
    .option("--hook-token <token>", "Neuroion hook token")
