...
function appendNeuroionHubHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("neuroion skills list")}\` to see all skills.`
      : "No skills found.";
    return appendNeuroionHubHint(message, opts.json);
  }
  ...
  return appendNeuroionHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendNeuroionHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("neuroion skills list")}\` to see available skills.`,
      opts.json,
    );
  }
...
export function registerSkillsCli(program: Command) {
  const skills = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/skills", "docs.neuroion.ai/cli/skills")}\n`,
    );
