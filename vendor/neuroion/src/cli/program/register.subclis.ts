...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.NEUROION_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.NEUROION_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<NeuroionConfig> => {
