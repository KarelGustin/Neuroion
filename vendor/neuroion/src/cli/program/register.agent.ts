...
${formatHelpExamples([
  ['neuroion agent --to +15555550123 --message "status update"', "Start a new session."],
  ['neuroion agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'neuroion agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'neuroion agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['neuroion agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'neuroion agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.neuroion.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.neuroion.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['neuroion agents set-identity --agent main --name "Neuroion" --emoji "🦞"', "Set name + emoji."],
  ["neuroion agents set-identity --agent main --avatar avatars/neuroion.png", "Set avatar path."],
  [
    "neuroion agents set-identity --workspace ~/.neuroion/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "neuroion agents set-identity --identity-file ~/.neuroion/workspace/IDENTITY.md --agent main",
