...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.NEUROION_GATEWAY_PORT).toBe("18789");
    expect(env.NEUROION_GATEWAY_TOKEN).toBe("secret");
    expect(env.NEUROION_SERVICE_MARKER).toBe("neuroion");
    expect(env.NEUROION_SERVICE_KIND).toBe("gateway");
    expect(typeof env.NEUROION_SERVICE_VERSION).toBe("string");
    expect(env.NEUROION_SYSTEMD_UNIT).toBe("neuroion-gateway.service");
    if (process.platform === "darwin") {
      expect(env.NEUROION_LAUNCHD_LABEL).toBe("ai.neuroion.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", NEUROION_PROFILE: "work" },
      port: 18789,
    });
    expect(env.NEUROION_SYSTEMD_UNIT).toBe("neuroion-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.NEUROION_LAUNCHD_LABEL).toBe("ai.neuroion.work");
    }
  });
});
