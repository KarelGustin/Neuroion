...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "neuroion" | "neuroionbot" | "neuroionbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["neuroion", "neuroionbot", "neuroionbot"] as const;
const execFileAsync = promisify(execFile);

export function renderGatewayServiceCleanupHints(
  env: Record<string, string | undefined> = process.env as Record<string, string | undefined>,
): string[] {
  const profile = env.NEUROION_PROFILE;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["neuroion_service_marker"];
  const kindKeys = ["neuroion_service_kind"];
...
function isNeuroionGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.neuroion.");
}

function isNeuroionGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("neuroion-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isNeuroionGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) {
    return false;
  }
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("neuroion gateway");
}
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("neuroionbot") || lower.includes("neuroionbot");
}
  ...
  for (const entry of entries) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "neuroionbot" : "neuroionbot",
        legacy: true,
      });
      continue;
    }
    ...
    if (marker === "neuroion" && isNeuroionGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "neuroion" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "neuroion" && isNeuroionGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isNeuroionGatewayTaskName(name)) {
        continue;
      }
