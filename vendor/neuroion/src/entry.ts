...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.NEUROION_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.NEUROION_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.NEUROION_NODE_OPTIONS_READY = "1";
  ...
  child.once("error", (error) => {
    console.error(
      "[neuroion] Failed to respawn CLI:",
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[neuroion] ${parsed.error}`);
    process.exit(2);
  }
