...
const execHostEnforced = process.env.NEUROION_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.NEUROION_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureNeuroionCliOnPath({ pathEnv: process.env.PATH ?? "" });
