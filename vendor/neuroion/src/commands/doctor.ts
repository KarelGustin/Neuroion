...
function resolveMode(cfg: NeuroionConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Neuroion doctor");

  const root = await resolveNeuroionPackageRoot({
  ...
  let cfg: NeuroionConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("neuroion configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("neuroion config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("neuroion setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("neuroion doctor --fix")}" to apply changes.`);
  }
