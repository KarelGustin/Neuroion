...
const SEARCH_TOOL = "https://docs.neuroion.ai/mcp.SearchNeuroion";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.neuroion.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('neuroion docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.neuroion.ai/");
      runtime.log(`Search: ${formatCliCommand('neuroion docs "your query"')}`);
    }
    return;
  }
