...
beforeEach(() => {
  ...
  resolveNeuroionPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.NEUROION_STATE_DIR;
  originalUpdateInProgress = process.env.NEUROION_UPDATE_IN_PROGRESS;
  process.env.NEUROION_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "neuroion-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.NEUROION_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.NEUROION_UPDATE_IN_PROGRESS;
  } else {
    process.env.NEUROION_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveNeuroionPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadNeuroionPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/neuroion-root.js", () => ({
  resolveNeuroionPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "neuroion-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.NEUROION_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when NEUROION_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/neuroion.json",
      exists: true,
      raw: "{}",
    ...
    process.env.NEUROION_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
