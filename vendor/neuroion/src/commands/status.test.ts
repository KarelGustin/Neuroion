...
beforeAll(() => {
  previousProfile = process.env.NEUROION_PROFILE;
  process.env.NEUROION_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.NEUROION_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/neuroion-root.js", () => ({
  resolveNeuroionPackageRoot: vi.fn().mockResolvedValue("/tmp/neuroion"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/neuroion/pnpm-lock.yaml",
      markerPath: "/tmp/neuroion/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Neuroion status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("neuroion status --all") ||
          l.includes("neuroion --profile isolated status --all") ||
          l.includes("neuroion status --all") ||
          l.includes("neuroion --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.NEUROION_GATEWAY_TOKEN = "abcd1234";
    try {
