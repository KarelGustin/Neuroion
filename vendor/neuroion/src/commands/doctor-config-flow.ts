...
function stripUnknownConfigKeys(config: NeuroionConfig): {
  config: NeuroionConfig;
  removed: string[];
} {
  const parsed = NeuroionSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: NeuroionConfig) {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".neuroion");
  const targetPath = path.join(targetDir, "neuroion.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: NeuroionConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("neuroion doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("neuroion doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "neuroion doctor --fix" to remove these keys.');
    }
  }
