...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "NEUROION_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("NEUROION_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv NEUROION_GATEWAY_TOKEN");
    expect(message).not.toContain("NEUROION_GATEWAY_PASSWORD");
  });
