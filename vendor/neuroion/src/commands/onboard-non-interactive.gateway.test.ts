...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.NEUROION_GATEWAY_PASSWORD,
  };
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.NEUROION_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "neuroion-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.NEUROION_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "neuroion");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.NEUROION_STATE_DIR = stateDir;
    process.env.NEUROION_CONFIG_PATH = path.join(stateDir, "neuroion.json");
