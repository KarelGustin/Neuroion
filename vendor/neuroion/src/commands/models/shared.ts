...
export async function updateConfig(
  mutator: (cfg: NeuroionConfig) => NeuroionConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: NeuroionConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: NeuroionConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("neuroion agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
