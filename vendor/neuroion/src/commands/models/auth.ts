...
import { readConfigFileSnapshot, type NeuroionConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: NeuroionConfig, model: string): NeuroionConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("neuroion plugins install")}\`.`,
    );
  }
