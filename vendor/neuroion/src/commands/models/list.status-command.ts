...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveNeuroionAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("neuroion models auth setup-token")}\` or \`${formatCliCommand("neuroion configure")}\`.`
          : `Run \`${formatCliCommand("neuroion configure")}\` or set an API key env var.`;
