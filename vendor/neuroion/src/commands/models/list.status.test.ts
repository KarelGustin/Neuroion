...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveNeuroionAgentDir: vi.fn().mockReturnValue("/tmp/neuroion-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/neuroion-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/neuroion-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveNeuroionAgentDir: mocks.resolveNeuroionAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveNeuroionAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/neuroion-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/neuroion-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/neuroion-agent-custom");
