...
export function listAgentEntries(cfg: NeuroionConfig): AgentEntry[] {
...
function resolveAgentName(cfg: NeuroionConfig, agentId: string) {
...
function resolveAgentModel(cfg: NeuroionConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: NeuroionConfig): AgentSummary[] {
