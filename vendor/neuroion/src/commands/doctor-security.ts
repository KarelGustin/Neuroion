...
import type { NeuroionConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: NeuroionConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("neuroion security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("neuroion configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("neuroion config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("neuroion doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("neuroion config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
