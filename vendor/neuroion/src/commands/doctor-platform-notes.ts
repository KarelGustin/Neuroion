...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".neuroion", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: NeuroionConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const deprecatedLaunchctlEntries = [
    ["NEUROIONBOT_GATEWAY_TOKEN", await getenv("NEUROIONBOT_GATEWAY_TOKEN")],
    ["NEUROIONBOT_GATEWAY_PASSWORD", await getenv("NEUROIONBOT_GATEWAY_PASSWORD")],
    ["NEUROIONBOT_GATEWAY_TOKEN", await getenv("NEUROIONBOT_GATEWAY_TOKEN")],
    ["NEUROIONBOT_GATEWAY_PASSWORD", await getenv("NEUROIONBOT_GATEWAY_PASSWORD")],
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`NEUROION_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
  ...
  const tokenEntries = [
    ["NEUROION_GATEWAY_TOKEN", await getenv("NEUROION_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["NEUROION_GATEWAY_PASSWORD", await getenv("NEUROION_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "NEUROION_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const entries = Object.entries(env)
    .filter(
      ([key, value]) =>
        (key.startsWith("NEUROIONBOT_") || key.startsWith("NEUROIONBOT_")) && value?.trim(),
    )
    .map(([key]) => key);
  if (entries.length === 0) {
    return;
  ...
  const lines = [
    ...
    "- Use NEUROION_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> NEUROION_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
