...
const requireAgentDir = () => {
  const agentDir = process.env.NEUROION_AGENT_DIR;
  if (!agentDir) {
    throw new Error("NEUROION_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.NEUROION_STATE_DIR;
  const previousAgentDir = process.env.NEUROION_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.NEUROION_AGENT_DIR;
    } else {
      process.env.NEUROION_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.NEUROION_OAUTH_DIR;
  });

  it("writes auth-profiles.json under NEUROION_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "neuroion-oauth-"));
    process.env.NEUROION_STATE_DIR = tempStateDir;
    process.env.NEUROION_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.NEUROION_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to NEUROION_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "neuroion-minimax-"));
    process.env.NEUROION_STATE_DIR = tempStateDir;
    process.env.NEUROION_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.NEUROION_AGENT_DIR;
