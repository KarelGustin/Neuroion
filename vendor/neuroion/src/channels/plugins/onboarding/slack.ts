...
function setSlackDmPolicy(cfg: NeuroionConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Neuroion";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Neuroion`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: NeuroionConfig, allowFrom: string[]): NeuroionConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
