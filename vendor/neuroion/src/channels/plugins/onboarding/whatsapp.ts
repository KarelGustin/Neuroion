...
function setWhatsAppDmPolicy(cfg: NeuroionConfig, dmPolicy: DmPolicy): NeuroionConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: NeuroionConfig, allowFrom?: string[]): NeuroionConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: NeuroionConfig, selfChatMode: boolean): NeuroionConfig {
...
async function detectWhatsAppLinked(cfg: NeuroionConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so Neuroion can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Neuroion" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("neuroion channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
