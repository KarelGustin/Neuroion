...
function resolveActiveHoursTimezone(cfg: NeuroionConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: NeuroionConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: NeuroionConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: NeuroionConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: NeuroionConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: NeuroionConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: NeuroionConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: NeuroionConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
