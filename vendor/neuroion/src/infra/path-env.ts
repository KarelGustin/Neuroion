...
function candidateBinDirs(opts: EnsureNeuroionPathOpts): string[] {
  ...
  // Bundled macOS app: `neuroion` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "neuroion");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/neuroion` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "neuroion"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `neuroion` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureNeuroionCliOnPath(opts: EnsureNeuroionPathOpts = {}) {
  if (isTruthyEnvValue(process.env.NEUROION_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.NEUROION_PATH_BOOTSTRAPPED = "1";
