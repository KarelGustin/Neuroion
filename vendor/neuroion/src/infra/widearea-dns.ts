...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.NEUROION_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*neuroion-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "neuroion";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "neuroion");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "neuroion-gw");
  ...
  records.push(`_neuroion-gw._tcp IN PTR ${instanceLabel}._neuroion-gw._tcp`);
  records.push(`${instanceLabel}._neuroion-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._neuroion-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; neuroion-content-hash: ${contentHash}\n${contentBody}`;
}
