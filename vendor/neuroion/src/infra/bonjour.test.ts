...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.NEUROION_MDNS_HOSTNAME = "test-host";
    process.env.NEUROION_MDNS_HOSTNAME = "test-host";
    ...
    expect(gatewayCall?.[0]?.type).toBe("neuroion-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("neuroion (Neuroion)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("neuroion");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("neuroion.local");
