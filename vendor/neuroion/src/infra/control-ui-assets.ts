...
import { resolveNeuroionPackageRoot, resolveNeuroionPackageRootSync } from "./neuroion-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveNeuroionPackageRoot({ argv1: normalized });
  if (!packageRoot) {
    return null;
  }
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveNeuroionPackageRootSync({
    argv1,
  ...
  if (argv1Dir) {
    // neuroion.mjs or dist/<bundle>.js
