...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.NEUROION_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Neuroion";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Neuroion\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.NEUROION_MDNS_HOSTNAME?.trim() ||
    process.env.NEUROIONBOT_MDNS_HOSTNAME?.trim() ||
    "neuroion";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
