...
export function triggerNeuroionRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.NEUROION_SYSTEMD_UNIT,
        process.env.NEUROION_PROFILE,
      );
  ...
  const label =
    process.env.NEUROION_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.NEUROION_PROFILE);
