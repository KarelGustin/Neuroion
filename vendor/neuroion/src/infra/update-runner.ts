...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "neuroion-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "neuroion doctor",
        managerScriptArgs(manager, "neuroion", ["doctor", "--non-interactive"]),
        gitRoot,
        { NEUROION_UPDATE_IN_PROGRESS: "1" },
      ),
    );
