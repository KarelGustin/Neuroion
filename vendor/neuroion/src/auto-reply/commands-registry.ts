...
import type { NeuroionConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: NeuroionConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: NeuroionConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as NeuroionConfig),
...
export function getCommandDetection(_cfg?: NeuroionConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: NeuroionConfig) {
