...
import { scheduleGatewaySigusr1Restart, triggerNeuroionRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Neuroion in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerNeuroionRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Neuroion via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
