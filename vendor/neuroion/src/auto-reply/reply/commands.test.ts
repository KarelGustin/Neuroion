...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "neuroion-commands-"));
...
function buildParams(commandBody: string, cfg: NeuroionConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
