...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  it("retries after compaction failure by resetting the session", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "neuroion-session-compaction-reset-"));
  ...
  it("retries after context overflow payload by resetting the session", async () => {
    const prevStateDir = process.env.NEUROION_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "neuroion-session-overflow-reset-"));
    process.env.NEUROION_STATE_DIR = stateDir;
    try {
      const sessionId = "session";
      const storePath = path.join(stateDir, "sessions", "sessions.json");
  ...
  it("resets the session after role ordering payloads", async () => {
    const prevStateDir = process.env.NEUROION_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "neuroion-session-role-ordering-"));
    process.env.NEUROION_STATE_DIR = stateDir;
    try {
      const sessionId = "session";
      const storePath = path.join(stateDir, "sessions", "sessions.json");
