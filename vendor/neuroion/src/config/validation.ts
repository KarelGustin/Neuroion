...
import type { NeuroionConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: NeuroionConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: NeuroionConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = NeuroionSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as NeuroionConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as NeuroionConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as NeuroionConfig)),
    ),
  };
}
