...
import type { NeuroionConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareNeuroionVersions } from "./version.js";
...
function coerceConfig(value: unknown): NeuroionConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as NeuroionConfig;
}
...
function stampConfigVersion(cfg: NeuroionConfig): NeuroionConfig {
...
function warnIfConfigFromFuture(cfg: NeuroionConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareNeuroionVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Neuroion (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: NeuroionConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): NeuroionConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as NeuroionConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as NeuroionConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: NeuroionConfig) {
    clearConfigCache();
...
// module scope. `NEUROION_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.NEUROION_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.NEUROION_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): NeuroionConfig {
...
export async function writeConfigFile(cfg: NeuroionConfig): Promise<void> {
  clearConfigCache();
