...
const ETC_NEUROION_DIR = path.join(ROOT_DIR, "etc", "neuroion");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "neuroion.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcNeuroionPath(...parts: string[]) {
  return path.join(ETC_NEUROION_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcNeuroionPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "neuroion.json"))).toEqual({
      shared: true,
    });
  });
});
