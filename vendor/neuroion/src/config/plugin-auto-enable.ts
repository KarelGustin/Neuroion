import type { NeuroionConfig } from "./config.js";
...
function isTelegramConfigured(cfg: NeuroionConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: NeuroionConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: NeuroionConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: NeuroionConfig): boolean {
...
function isIMessageConfigured(cfg: NeuroionConfig): boolean {
...
function isWhatsAppConfigured(cfg: NeuroionConfig): boolean {
...
function isGenericChannelConfigured(cfg: NeuroionConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: NeuroionConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: NeuroionConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: NeuroionConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: NeuroionConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: NeuroionConfig, pluginId: string): NeuroionConfig {
...
function enablePluginEntry(cfg: NeuroionConfig, pluginId: string): NeuroionConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
