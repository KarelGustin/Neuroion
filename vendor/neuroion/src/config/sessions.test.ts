...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-neuroion",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-neuroion#general");
  });
  ...
  it("updateLastRoute persists channel and target", async () => {
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "neuroion-sessions-"));
  ...
  it("derives session transcripts dir from NEUROION_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { NEUROION_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.NEUROION_STATE_DIR = "/custom/state";
    try {
