...
import type { NeuroionConfig, ConfigFileSnapshot } from "../config/config.js";
...
function summarizeGroupPolicy(cfg: NeuroionConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
export function collectAttackSurfaceSummaryFindings(cfg: NeuroionConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep NEUROION_STATE_DIR on a local-only volume and re-run "${formatCliCommand("neuroion security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: NeuroionConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer NEUROION_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: NeuroionConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectModels(cfg: NeuroionConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
export function collectModelHygieneFindings(cfg: NeuroionConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: NeuroionConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: NeuroionConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: NeuroionConfig): boolean {
...
function isBrowserEnabled(cfg: NeuroionConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: NeuroionConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectExposureMatrixFindings(cfg: NeuroionConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
