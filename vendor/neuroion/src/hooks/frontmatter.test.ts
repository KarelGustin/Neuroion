...
`;
    ...
    expect(parsed.neuroion.emoji).toBe("💾");
    expect(parsed.neuroion.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.neuroion.emoji).toBe("📝");
    expect(parsed.neuroion.events).toEqual(["command"]);
    expect(parsed.neuroion.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.neuroion.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"neuroion": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"neuroion": {"events": ["test"]}}');
  });
...
describe("resolveNeuroionMetadata", () => {
  it("extracts neuroion metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveNeuroionMetadata(frontmatter);
  ...
  it("returns undefined when neuroion key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        neuroion: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Neuroion" },
            { id: "npm", kind: "npm", package: "@neuroion/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@neuroion/hook");
  });
    ...
    "neuroion":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Neuroion" }],
      },
  }
---
...
`;
    ...
    const neuroion = resolveNeuroionMetadata(frontmatter);
    expect(neuroion).toBeDefined();
    expect(neuroion?.emoji).toBe("💾");
    expect(neuroion?.events).toEqual(["command:new"]);
    expect(neuroion?.requires?.config).toEqual(["workspace.dir"]);
    expect(neuroion?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const neuroion = resolveNeuroionMetadata(frontmatter);
    expect(neuroion?.emoji).toBe("disk");
    expect(neuroion?.events).toEqual(["command:new"]);
  });
});
