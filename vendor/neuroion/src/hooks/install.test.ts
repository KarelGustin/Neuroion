...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `neuroion-hook-install-${randomUUID()}`);
...
describe("installHooksFromArchive", () => {
  it("installs hook packs from zip archives", async () => {
    ...
    zip.file(
      ...
      JSON.stringify({
        name: "@neuroion/zip-hooks",
        version: "0.0.1",
        neuroion: { hooks: ["./hooks/zip-hook"] },
      }),
    );
    zip.file(
  ...
  it("installs hook packs from tar archives", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@neuroion/tar-hooks",
        version: "0.0.1",
        neuroion: { hooks: ["./hooks/tar-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("rejects hook packs with traversal-like ids", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@evil/..",
        version: "0.0.1",
        neuroion: { hooks: ["./hooks/evil-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("rejects hook packs with reserved ids", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@evil/.",
        version: "0.0.1",
        neuroion: { hooks: ["./hooks/reserved-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
