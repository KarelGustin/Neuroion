...
homepage: https://docs.neuroion.ai/hooks#command-logger
metadata:
  {
    "neuroion":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.neuroion/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.neuroion/logs/commands.log ~/.neuroion/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/neuroion`:
   ```
   /home/username/.neuroion/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
