...
This directory contains hooks that ship with Neuroion. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.neuroion.ai/hooks/soul-evil
...
Hooks can be configured in `~/.neuroion/neuroion.json`:
...
2. Restart gateway: `pkill -9 -f 'neuroion.*gateway' && pnpm neuroion gateway`
3. Enable the hook: `neuroion hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.neuroion.ai/hooks
