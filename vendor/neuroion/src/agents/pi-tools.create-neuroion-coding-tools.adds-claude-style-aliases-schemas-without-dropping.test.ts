...
import { __testing, createNeuroionCodingTools } from "./pi-tools.js";
...
describe("createNeuroionCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createNeuroionTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createNeuroionCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "neuroion-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "neuroion-outside.txt");
