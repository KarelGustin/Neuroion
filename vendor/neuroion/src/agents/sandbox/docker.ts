...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "neuroion.sandbox=1");
  args.push("--label", `neuroion.sessionKey=${params.scopeKey}`);
  args.push("--label", `neuroion.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `neuroion.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("neuroion.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`neuroion sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`neuroion sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("neuroion sandbox recreate --all");
}
