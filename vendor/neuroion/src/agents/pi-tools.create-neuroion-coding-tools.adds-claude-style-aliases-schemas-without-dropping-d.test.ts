...
const defaultTools = createNeuroionCodingTools();

describe("createNeuroionCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "neuroion-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createNeuroionCodingTools();
    ...
    try {
      ...
      const contents = "Hello from neuroion read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "neuroion-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "neuroion-workspace"),
    ...
    const tools = createNeuroionCodingTools({ sandbox });
