...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "neuroion-embedded-agent-"));
...
const ensureModels = (cfg: NeuroionConfig) => ensureNeuroionModelsJson(cfg, agentDir) as unknown;
