...
describe("Agent-specific sandbox config", () => {
  ...
  beforeEach(async () => {
    spawnCalls.length = 0;
    previousStateDir = process.env.NEUROIONBOT_STATE_DIR;
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "neuroionbot-test-state-"));
    process.env.NEUROIONBOT_STATE_DIR = tempStateDir;
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (previousStateDir === undefined) {
      delete process.env.NEUROIONBOT_STATE_DIR;
    } else {
      process.env.NEUROIONBOT_STATE_DIR = previousStateDir;
    }
