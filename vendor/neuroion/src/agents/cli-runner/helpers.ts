...
/**
 * Cleanup suspended Neuroion CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: NeuroionConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "neuroion-cli-images-"));
  const paths: string[] = [];
