...
import { createNeuroionCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("neuroion-ws-", async (workspaceDir) => {
      await withTempDir("neuroion-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createNeuroionCodingTools({ workspaceDir });
    ...
    await withTempDir("neuroion-ws-", async (workspaceDir) => {
      await withTempDir("neuroion-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello neuroion");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("neuroion-ws-", async (workspaceDir) => {
      await withTempDir("neuroion-override-", async (overrideDir) => {
        const tools = createNeuroionCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("neuroion-sandbox-", async (sandboxDir) => {
      await withTempDir("neuroion-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "neuroion-sbx-test",
        ...
        const tools = createNeuroionCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
