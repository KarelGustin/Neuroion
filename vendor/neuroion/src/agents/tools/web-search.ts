...
type WebSearchConfig = NonNullable<NeuroionConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: NeuroionConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("neuroion configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.neuroion.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://neuroion.ai",
      "X-Title": "Neuroion Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
