...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "NEUROION_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "neuroion-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.NEUROION_STATE_DIR = path.join(home, ".neuroion");
    ...
    await fs.mkdir(path.join(home, ".neuroion"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.neuroion${path.sep}media`);
