...
import { loadNeuroionPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.NEUROION_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `neuroion-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "neuroion.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.NEUROION_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadNeuroionPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.NEUROION_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadNeuroionPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadNeuroionPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
