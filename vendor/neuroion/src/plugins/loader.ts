...
import { discoverNeuroionPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: NeuroionPluginDefinition;
  register?: NeuroionPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as NeuroionPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as NeuroionPluginDefinition;
...
export function loadNeuroionPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverNeuroionPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "neuroion/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: NeuroionPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as NeuroionPluginModule;
    } catch (err) {
