...
`neuroion doctor` is the repair + migration tool for Neuroion. It fixes stale
...
- Extra workspace dir detection (`~/neuroion`).
...
you to run `neuroion doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.neuroion/neuroion.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.neuroion/sessions/` to `~/.neuroion/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.neuroion/agent/` to `~/.neuroion/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.neuroion/credentials/*.json` (except `oauth.json`)
  - to `~/.neuroion/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `neuroion doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.neuroion` folders exist across
  home directories or when `NEUROION_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.neuroion/neuroion.json` is
...
offers to remove them and install the Neuroion service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Neuroion gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `neuroion doctor --generate-gateway-token` to force token
...
- `neuroion doctor` prompts before rewriting supervisor config.
- `neuroion doctor --yes` accepts the default repair prompts.
- `neuroion doctor --repair` applies recommended fixes without prompts.
- `neuroion doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `neuroion gateway install --force`.
