...
- Replaces the legacy `gateway` command. CLI entry point: `neuroion gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)

```bash
neuroion gateway --port 18789
# for full debug/trace logs in stdio:
neuroion gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
neuroion gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.neuroion/neuroion.json` (or `NEUROION_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__neuroion__/canvas/` from `~/.neuroion/workspace/canvas`. Disable with `canvasHost.enabled=false` or `NEUROION_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `NEUROION_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `NEUROION_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.neuroion.*` may still exist)
- Linux: `neuroion-gateway-<profile>.service`
- Windows: `Neuroion Gateway (<profile>)`

Install metadata is embedded in the service config:

- `NEUROION_SERVICE_MARKER=neuroion`
- `NEUROION_SERVICE_KIND=gateway`
- `NEUROION_SERVICE_VERSION=<version>`
...
```bash
neuroion --dev setup
neuroion --dev gateway --allow-unconfigured
...
- `NEUROION_STATE_DIR=~/.neuroion-dev`
- `NEUROION_CONFIG_PATH=~/.neuroion-dev/neuroion.json`
- `NEUROION_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.neuroion/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `NEUROION_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `NEUROION_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `NEUROION_CONFIG_PATH`
- unique `NEUROION_STATE_DIR`
...
```bash
neuroion --profile main gateway install
neuroion --profile rescue gateway install
```

Example:

```bash
NEUROION_CONFIG_PATH=~/.neuroion/a.json NEUROION_STATE_DIR=~/.neuroion-a neuroion gateway --port 19001
NEUROION_CONFIG_PATH=~/.neuroion/b.json NEUROION_STATE_DIR=~/.neuroion-b neuroion gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `neuroion health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `neuroion`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `neuroion gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.neuroion.*` is cleaned up).
  - `neuroion doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Neuroion profile services.
  ...
  - Cleanup: `neuroion gateway uninstall` (current service) and `neuroion doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `neuroion gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- Neuroion.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.neuroion.*` labels still unload cleanly).
- To stop it cleanly, use `neuroion gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `neuroion gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `neuroion gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

Neuroion installs a **systemd user service** by default on Linux/WSL2. We
...
`neuroion gateway install` writes the user unit. `neuroion doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/neuroion-gateway[-<profile>].service`:

```
[Unit]
Description=Neuroion Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/neuroion gateway --port 18789
Restart=always
RestartSec=5
Environment=NEUROION_GATEWAY_TOKEN=
...
```
systemctl --user enable --now neuroion-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/neuroion-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now neuroion-gateway[-<profile>].service
```
...
- `neuroion gateway health|status` — request health/status over the Gateway WS.
- `neuroion message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `neuroion agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `neuroion gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `neuroion gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `neuroion gateway` and the legacy TCP control port.
