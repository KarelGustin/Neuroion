---
summary: "Quick troubleshooting guide for common Neuroion failures"
read_when:
...
When Neuroion misbehaves, here's how to fix it.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `neuroion status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `neuroion status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `neuroion status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `neuroion gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `neuroion channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `neuroion gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `neuroion logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `neuroion status --all` (it redacts tokens). If you paste `neuroion status`, consider setting `NEUROION_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  neuroion models auth setup-token --provider anthropic
  ```
...
```bash
neuroion models auth paste-token --provider anthropic
neuroion models status
```
...
- Preferred: `neuroion logs --follow`
- File logs (always): `/tmp/neuroion/neuroion-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$NEUROION_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u neuroion-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  neuroion config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  neuroion config set gateway.mode remote
  neuroion config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `neuroion setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.neuroion/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `neuroion doctor`
...
- then run `neuroion sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `neuroion gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `neuroion gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `neuroion gateway status` and `neuroion doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `NEUROION_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `neuroion gateway status` shows a config mismatch**
...
- Fix: rerun `neuroion gateway install --force` from the same `--profile` / `NEUROION_STATE_DIR` you want the service to use.

**If `neuroion gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `neuroion doctor` to update it (or `neuroion gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `NEUROION_GATEWAY_TOKEN`) and restart the service.

**If `neuroion gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/neuroion` on disk.
...
Symptoms: `pwd` or file tools show `~/.neuroion/sandboxes/...` even though you
...
Neuroion intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `neuroion models list` or
  `neuroion models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${NEUROION_CONFIG_PATH:-$HOME/.neuroion/neuroion.json}"
```
...
tail -f "$(ls -t /tmp/neuroion/neuroion-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
neuroion logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.neuroion/agents/<agentId>/sessions/
```
...
neuroion logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
neuroion gateway --verbose
```

If you’re logged out / unlinked:

```bash
neuroion channels logout
trash "${NEUROION_STATE_DIR:-$HOME/.neuroion}/credentials" # if logout can't cleanly remove everything
neuroion channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/neuroion/neuroion-*.log | head -1)" | tail -20
```

### High Memory Usage

Neuroion keeps conversation history in memory.
...
Neuroion now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `neuroion doctor` reports every invalid entry.
- `neuroion doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `neuroion logs`, `neuroion health`, `neuroion status`, `neuroion gateway status`, and `neuroion gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/neuroion/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `neuroion models status` (CLI).
...
```bash
curl -fsSL https://neuroion.ai/install.sh | bash -s -- --install-method git --no-onboard
```
...
5. Run `neuroion channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Neuroion scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/neuroion/neuroion/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.neuroion.* still works)
```
...
Ensure the global `neuroion` CLI is installed and matches the app version:

```bash
neuroion --version
npm install -g neuroion@<version>
```
...
#   ${NEUROION_CONFIG_PATH:-$HOME/.neuroion/neuroion.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
neuroion gateway --verbose
neuroion channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/neuroion/neuroion-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$NEUROION_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.neuroion/logs/...`; profiles use `~/.neuroion-<profile>/logs/...`)<br />Linux: `journalctl --user -u neuroion-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Neuroion Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$NEUROION_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                            |
| Media cache                       | `$NEUROION_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                |
| Credentials                       | `$NEUROION_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                          |
...
neuroion gateway status --deep

# Is the gateway reachable?
neuroion health --json
# If it fails, rerun with connection details:
neuroion health --verbose
...
tail -20 /tmp/neuroion/neuroion-*.log
```
...
# neuroion gateway uninstall

trash "${NEUROION_STATE_DIR:-$HOME/.neuroion}"
neuroion channels login         # re-pair WhatsApp
neuroion gateway restart           # or: neuroion gateway
```
...
1. Check logs first: `/tmp/neuroion/` (default: `neuroion-YYYY-MM-DD.log`, or your configured `logging.file`)
