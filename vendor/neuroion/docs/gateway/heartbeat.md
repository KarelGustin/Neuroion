...
- During heartbeat runs, Neuroion treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `neuroion sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Neuroion skips the heartbeat run entirely (no model call).
...
```bash
neuroion system event --text "Check for urgent follow-ups" --mode now
```
