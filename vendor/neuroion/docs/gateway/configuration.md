---
summary: "All configuration options for ~/.neuroion/neuroion.json with examples"
read_when:
...
Neuroion reads an optional **JSON5** config from `~/.neuroion/neuroion.json` (comments + trailing commas allowed).

If the file is missing, Neuroion uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.neuroion/workspace`). You usually only need a config to:
...
Neuroion only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `neuroion doctor`, `neuroion logs`, `neuroion health`, `neuroion status`, `neuroion service`, `neuroion help`).
- Run `neuroion doctor` to see the exact issues.
- Run `neuroion doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `neuroion config set`. Keep a backup of `~/.neuroion/neuroion.json`.
...
```bash
neuroion gateway call config.get --params '{}' # capture payload.hash
neuroion gateway call config.apply --params '{
...
```bash
neuroion gateway call config.get --params '{}' # capture payload.hash
neuroion gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.neuroion/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@neuroion", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.neuroion/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.neuroion/workspace" }],
}
```
...
{ "$include": "/etc/neuroion/base.json5" } // absolute
...
```json5
// ~/.neuroion/clients/mueller/agents.json5
[
...
```json5
// ~/.neuroion/clients/mueller/broadcast.json5
{
...
Neuroion reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Neuroion runs your login shell and imports only the missing expected keys (never overrides).
...
- `NEUROION_LOAD_SHELL_ENV=1`
- `NEUROION_SHELL_ENV_TIMEOUT_MS=15000`
...
Neuroion stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.neuroion/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.neuroion/credentials/oauth.json` (or `$NEUROION_STATE_DIR/credentials/oauth.json`)
...
- `~/.neuroion/agent/*` (migrated by `neuroion doctor` into `~/.neuroion/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `NEUROION_OAUTH_DIR`
- Agent dir (default agent root override): `NEUROION_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Neuroion imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Neuroion derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/neuroion/neuroion-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/neuroion/neuroion.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/neuroion/neuroion.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.neuroion/credentials/whatsapp/biz
          // authDir: "~/.neuroion/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `neuroion doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@neuroion", "neuroion"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@neuroion"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.neuroion/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.neuroion/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.neuroion/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.neuroion/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.neuroion/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.neuroion/workspace-home" },
      { id: "work", workspace: "~/.neuroion/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `neuroion.json`).
...
Neuroion starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["neuroion-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-neuroion",
...
Neuroion starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `neuroion plugins install @neuroion/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Neuroion starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Neuroion spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Neuroion
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/neuroion" } },
}
```
...
When a file exceeds this limit, Neuroion logs a warning and injects a truncated
...
message envelopes). If unset, Neuroion uses the host timezone at runtime.
...
`[neuroion]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[neuroion]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[neuroion]"`, Neuroion will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Neuroion generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.neuroion/settings/tts.json",
      elevenlabs: {
...
`voiceAliases` lets Talk directives use friendly names (e.g. `"voice":"Neuroion"`).
...
Neuroion also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Neuroion currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `neuroion configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:neuroion`: all built-in Neuroion tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.neuroion/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.neuroion/sandboxes",
        docker: {
          image: "neuroion-sandbox:bookworm-slim",
          containerPrefix: "neuroion-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "neuroion-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "neuroion-sandbox-browser:bookworm-slim",
          containerPrefix: "neuroion-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Neuroion uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.neuroion/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Neuroion config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Neuroion writes/merges a `models.json` into
`~/.neuroion/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Neuroion uses
...
- Shortcut: `neuroion onboard --auth-choice opencode-zen`.
...
Shortcut: `neuroion onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `neuroion onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `neuroion onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `neuroion onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `neuroion onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `neuroion onboard --auth-choice minimax-api`.
...
- Override the agent config root with `NEUROION_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.neuroion/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.neuroion/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.neuroion/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Neuroion stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.neuroion/skills` (workspace skills
...
from `~/.neuroion/extensions`, `<workspace>/.neuroion/extensions`, plus any
...
### `browser` (neuroion-managed browser)

Neuroion can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for neuroion and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Neuroion.app menubar, or `neuroion gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      neuroion: { cdpPort: 18800, color: "#FF4500" },
...
- Examples: `"/ui"`, `"/neuroion"`, `"/apps/neuroion"`.
...
- When a connection comes from one of these IPs, Neuroion uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `neuroion gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `NEUROION_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `NEUROION_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Neuroion
...
- Neuroion.app watches `~/.neuroion/neuroion.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.neuroion/neuroion.json` (or `NEUROION_CONFIG_PATH`) and applies changes automatically.
...
- `~/.neuroion/neuroion.json` (or `NEUROION_CONFIG_PATH`)
...
- `NEUROION_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `neuroion --dev …` → uses `~/.neuroion-dev` + shifts ports from base `19001`
- `neuroion --profile <name> …` → uses `~/.neuroion-<name>` (port via config/env/flags)
...
```bash
NEUROION_CONFIG_PATH=~/.neuroion/a.json \
NEUROION_STATE_DIR=~/.neuroion-a \
neuroion gateway --port 19001
```
...
Gmail helper config (used by `neuroion webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "neuroion@gmail.com",
...
- Set `NEUROION_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Neuroion defaults `serve.path` to `/` so
...
Default root: `~/.neuroion/workspace/canvas`  
Default port: `18793` (chosen to avoid the neuroion browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__neuroion__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__neuroion__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
{
  canvasHost: {
    root: "~/.neuroion/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
- env: `NEUROION_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.neuroion/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.neuroion/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.neuroion/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_neuroion-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `neuroion` (advertises `neuroion.local`). Override with `NEUROION_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_neuroion-gw._tcp` under `~/.neuroion/dns/` using the configured discovery domain (example: `neuroion.internal.`).
