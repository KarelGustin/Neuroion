...
This document describes how Neuroion integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Neuroion uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Neuroion directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Neuroion's local TUI mode)                                             |
...
├── pi-tools.ts                    # createNeuroionCodingTools()
...
├── neuroion-tools.ts              # Neuroion-specific tools
...
2. **Custom Replacements**: Neuroion replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Neuroion Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Neuroion's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Neuroion CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Neuroion wraps this with `guardSessionManager()` for tool result safety.
...
Neuroion maintains an auth profile store with multiple API keys per provider:
...
Neuroion loads custom pi extensions for specialized behavior:
...
Neuroion also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Neuroion Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Neuroion tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.neuroion/agents/<agentId>/sessions/` (or `$NEUROION_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-tools.create-neuroion-coding-tools.adds-claude-style-aliases-schemas-without-dropping-b.test.ts`
- `src/agents/pi-tools.create-neuroion-coding-tools.adds-claude-style-aliases-schemas-without-dropping-d.test.ts`
- `src/agents/pi-tools.create-neuroion-coding-tools.adds-claude-style-aliases-schemas-without-dropping-f.test.ts`
- `src/agents/pi-tools.create-neuroion-coding-tools.adds-claude-style-aliases-schemas-without-dropping.test.ts`
