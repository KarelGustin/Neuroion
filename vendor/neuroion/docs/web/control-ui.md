...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/neuroion`)
...
If the page fails to load, start the Gateway first: `neuroion gateway`.
...
you revoke it with `neuroion devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.neuroion/neuroion.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Neuroion
...
Neuroion **blocks** Control UI connections without device identity.
