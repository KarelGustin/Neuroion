---
summary: "Research notes: offline memory system for Neuroion workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.neuroion/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Neuroion integration
...
Target: Neuroion-style workspace (`agents.defaults.workspace`, default `~/.neuroion/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.neuroion/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Neuroion to see every session.
...
```
~/.neuroion/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Neuroion**, but keep a separable core library.

### Why integrate into Neuroion?

- Neuroion already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `neuroion memory recall "…" --k 25 --since 30d`
  - `neuroion memory reflect --since 7d`
...
Pragmatic take for `~/.neuroion/workspace`:
