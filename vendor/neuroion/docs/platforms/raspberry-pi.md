---
summary: "Neuroion on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Neuroion on a Raspberry Pi
  - Running Neuroion on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Neuroion on Raspberry Pi

## Goal

Run a persistent, always-on Neuroion Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Neuroion features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled neuroion
...
```bash
# Check logs
journalctl -u neuroion --no-pager -n 100

# Common fix: rebuild
cd ~/neuroion  # if using hackable install
npm run build
sudo systemctl restart neuroion
```
