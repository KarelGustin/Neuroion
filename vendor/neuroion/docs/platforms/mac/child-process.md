...
service via the external `neuroion` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`NEUROION_PROFILE`; legacy `com.neuroion.*` is supported).
