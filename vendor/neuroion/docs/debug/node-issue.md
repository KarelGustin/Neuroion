...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Neuroion
...
Running Neuroion via Node with `tsx` fails at startup with:

```
[neuroion] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `neuroion status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch neuroion.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node neuroion.mjs status` works on Node 25.
