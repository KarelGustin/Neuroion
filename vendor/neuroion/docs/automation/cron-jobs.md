...
- Jobs persist under `~/.neuroion/cron/` so restarts don’t lose schedules.
...
neuroion cron list
neuroion cron run <job-id> --force
neuroion cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.neuroion/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `neuroion cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Neuroion defaults to `announce`.
...
- Job store: `~/.neuroion/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.neuroion/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
neuroion cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
neuroion cron edit <jobId> --agent ops
neuroion cron edit <jobId> --clear-agent
```
...
```bash
neuroion system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`neuroion system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `NEUROION_SKIP_CRON`.
