...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `neuroion sandbox explain`.
...
Legacy `agent.*` configs are migrated by `neuroion doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=neuroion-sbx-"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${NEUROION_STATE_DIR:-$HOME/.neuroion}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
