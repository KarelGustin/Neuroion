...
```bash
neuroion tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `neuroion tui --deliver`
...
- Check the Gateway logs: `neuroion logs --follow`.
- Confirm the agent can run: `neuroion status` and `neuroion models status`.
...
- No agents in picker: check `neuroion agents list` and your routing config.
