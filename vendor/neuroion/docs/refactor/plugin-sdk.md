...
- Publish as `neuroion/plugin-sdk` (or export from core under `neuroion/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `NeuroionPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: NeuroionConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: NeuroionConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: NeuroionConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: NeuroionConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: NeuroionConfig): string;
  };
};
```
...
- Introduce `neuroion/plugin-sdk`.
- Add `api.runtime` to `NeuroionPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `neuroionRuntime: ">=2026.2.0"`).
