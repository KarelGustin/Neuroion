...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.neuroion/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.neuroion/node.json` (node id + pairing token).
...
Path: `~/.neuroion/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.neuroion/exec-approvals.sock",
...
- Unix socket at `~/.neuroion/exec-approvals.sock` (0600).
