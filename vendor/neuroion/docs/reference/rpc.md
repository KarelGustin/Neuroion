...
Neuroion integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Neuroion owns lifecycle when `channels.signal.autoStart=true`.
...
- Neuroion spawns `imsg rpc` as a child process (legacy iMessage integration).
