...
Neuroion features that can generate provider usage or paid API calls.
...
- `neuroion status --usage` and `neuroion channels list` show provider **usage windows**
...
Neuroion can pick up credentials from:
...
- `neuroion status --usage`
- `neuroion models status --json`
...
`neuroion models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
