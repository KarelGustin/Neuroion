...
read_when:
  - You need detailed behavior for neuroion onboard
...
This page is the full reference for `neuroion onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.neuroion/neuroion.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `neuroion doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `neuroion pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `neuroion health`.
    - `neuroion status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.neuroion/.env` so launchd can read it.
...
- OAuth credentials: `~/.neuroion/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.neuroion/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`neuroion onboard`](/cli/onboard)
