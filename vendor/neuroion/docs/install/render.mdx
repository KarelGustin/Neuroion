...
Deploy Neuroion on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/neuroion/neuroion"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run Neuroion:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: NEUROION_STATE_DIR
        value: /data/.neuroion
      - key: NEUROION_WORKSPACE_DIR
        value: /data/workspace
      - key: NEUROION_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/neuroion`.
...
If you use the original Neuroion repository, Render will not auto-deploy your Neuroion. To update it, run a manual Blueprint sync from the dashboard.
...
For Neuroion, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Neuroion host.
