...
read_when:
  - "You installed Neuroion but `neuroion` is “command not found”"
...
Neuroion’s runtime baseline is **Node 22+**.

If you can run `npm install -g neuroion@latest` but later see `neuroion: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
