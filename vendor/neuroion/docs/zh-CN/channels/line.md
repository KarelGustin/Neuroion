...
LINE 通过 LINE Messaging API 连接到 Neuroion。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
neuroion plugins install @neuroion/line
```

本地检出（从 git 仓库运行时）：

```bash
neuroion plugins install ./extensions/line
```
...
```bash
neuroion pairing list line
neuroion pairing approve line <CODE>
```
