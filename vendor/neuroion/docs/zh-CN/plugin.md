...
read_when:
  - 添加或修改插件/扩展
  - 记录插件安装或加载规则
summary: Neuroion 插件/扩展：发现、配置和安全
title: 插件
x-i18n:
...
插件只是一个**小型代码模块**，用额外功能（命令、工具和 Gateway 网关 RPC）扩展 Neuroion。

大多数时候，当你想要一个尚未内置到核心 Neuroion 的功能（或你想将可选功能排除在主安装之外）时，你会使用插件。
...
- 从 2026.1.15 起 Microsoft Teams 仅作为插件提供；如果使用 Teams，请安装 `@neuroion/msteams`。
...
Neuroion 插件是通过 jiti 在运行时加载的 **TypeScript 模块**。**配置验证不会执行插件代码**；它使用插件清单和 JSON Schema。参见 [插件清单](/plugins/manifest)。
...
4. 捆绑扩展（随 Neuroion 一起发布，**默认禁用**）

- `<neuroion>/extensions/*`

捆绑插件必须通过 `plugins.entries.<id>.enabled` 或 `neuroion plugins enable <id>` 显式启用。已安装的插件默认启用，但可以用相同方式禁用。

每个插件必须在其根目录中包含 `neuroion.plugin.json` 文件。如果路径指向文件，则插件根目录是文件的目录，必须包含清单。

如果多个插件解析到相同的 id，上述顺序中的第一个匹配项获胜，较低优先级的副本被忽略。

### 包集合

插件目录可以包含带有 `neuroion.extensions` 的 `package.json`：
...
渠道插件可以通过 `neuroion.channel` 广播新手引导元数据，通过 `neuroion.install` 广播安装提示。这使核心目录保持无数据。
...
Neuroion 还可以合并**外部渠道目录**（例如，MPM 注册表导出）。将 JSON 文件放在以下位置之一：

- `~/.neuroion/mpm/plugins.json`
- `~/.neuroion/mpm/catalog.json`
- `~/.neuroion/plugins/catalog.json`

或将 `NEUROION_PLUGIN_CATALOG_PATHS`（或 `NEUROION_MPM_CATALOG_PATHS`）指向一个或多个 JSON 文件（逗号/分号/`PATH` 分隔）。每个文件应包含 `{ "entries": [ { "name": "@scope/pkg", "neuroion": { "channel": {...}, "install": {...} } } ] }`。
...
如果插件导出 `id`，Neuroion 会使用它，但当它与配置的 id 不匹配时会发出警告。
...
- 插件配置使用嵌入在 `neuroion.plugin.json`（`configSchema`）中的 JSON Schema 进行验证。
...
Neuroion 在运行时根据发现的插件增强 `uiHints`：
...
插件也可以注册自己的顶级命令（例如：`neuroion voicecall`）。
...
- 插件管理的钩子在 `neuroion hooks list` 中显示为 `plugin:<id>`。
- 你不能通过 `neuroion hooks` 启用/禁用插件管理的钩子；而是启用/禁用插件。

## 提供商插件（模型认证）

插件可以注册**模型提供商认证**流程，以便用户可以在 Neuroion 内运行 OAuth 或 API 密钥设置（无需外部脚本）。
...
- 主包：`neuroion`（本仓库）
- 插件：`@neuroion/*` 下的独立 npm 包（例如：`@neuroion/voice-call`）

发布契约：

- 插件 `package.json` 必须包含带有一个或多个入口文件的 `neuroion.extensions`。
- 入口文件可以是 `.js` 或 `.ts`（jiti 在运行时加载 TS）。
- `neuroion plugins install <npm-spec>` 使用 `npm pack`，提取到 `~/.neuroion/extensions/<id>/`，并在配置中启用它。
...
- 单独发布的插件应运行自己的 CI（lint/构建/测试）并验证 `neuroion.extensions` 指向构建的入口点（`dist/index.js`）。
