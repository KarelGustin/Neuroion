...
通过一键模板在 Railway 上部署 Neuroion，并在浏览器中完成设置。
...
- `https://<your-railway-domain>/neuroion` — 控制面板 UI

## 你将获得

- 托管的 Neuroion Gateway网关 + 控制面板 UI
- `/setup` 网页设置向导（无需终端命令）
...
- `NEUROION_STATE_DIR=/data/.neuroion`（推荐）
- `NEUROION_WORKSPACE_DIR=/data/workspace`（推荐）
- `NEUROION_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 Neuroion 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
