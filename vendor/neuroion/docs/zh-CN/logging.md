...
你可以在 `~/.neuroion/neuroion.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/neuroion.log"
  }
}
```
...
```bash
neuroion channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.neuroion/neuroion.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/neuroion/neuroion-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- Neuroion 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
NEUROION_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `neuroion plugins enable diagnostics-otel` 启用插件。
...
- `neuroion.tokens`（计数器，属性：`neuroion.token`、`neuroion.channel`、`neuroion.provider`、`neuroion.model`）
- `neuroion.cost.usd`（计数器，属性：`neuroion.channel`、`neuroion.provider`、`neuroion.model`）
- `neuroion.run.duration_ms`（直方图，属性：`neuroion.channel`、`neuroion.provider`、`neuroion.model`）
- `neuroion.context.tokens`（直方图，属性：`neuroion.context`、`neuroion.channel`、`neuroion.provider`、`neuroion.model`）

消息流：

- `neuroion.webhook.received`（计数器，属性：`neuroion.channel`、`neuroion.webhook`）
- `neuroion.webhook.error`（计数器，属性：`neuroion.channel`、`neuroion.webhook`）
- `neuroion.webhook.duration_ms`（直方图，属性：`neuroion.channel`、`neuroion.webhook`）
- `neuroion.message.queued`（计数器，属性：`neuroion.channel`、`neuroion.source`）
- `neuroion.message.processed`（计数器，属性：`neuroion.channel`、`neuroion.outcome`）
- `neuroion.message.duration_ms`（直方图，属性：`neuroion.channel`、`neuroion.outcome`）

队列 + 会话：

- `neuroion.queue.lane.enqueue`（计数器，属性：`neuroion.lane`）
- `neuroion.queue.lane.dequeue`（计数器，属性：`neuroion.lane`）
- `neuroion.queue.depth`（直方图，属性：`neuroion.lane` 或 `neuroion.channel=heartbeat`）
- `neuroion.queue.wait_ms`（直方图，属性：`neuroion.lane`）
- `neuroion.session.state`（计数器，属性：`neuroion.state`、`neuroion.reason`）
- `neuroion.session.stuck`（计数器，属性：`neuroion.state`）
- `neuroion.session.stuck_age_ms`（直方图，属性：`neuroion.state`）
- `neuroion.run.attempt`（计数器，属性：`neuroion.attempt`）

### 导出的 span（名称 + 关键属性）

- `neuroion.model.usage`
  - `neuroion.channel`、`neuroion.provider`、`neuroion.model`
  - `neuroion.sessionKey`、`neuroion.sessionId`
  - `neuroion.tokens.*`（input/output/cache_read/cache_write/total）
- `neuroion.webhook.processed`
  - `neuroion.channel`、`neuroion.webhook`、`neuroion.chatId`
- `neuroion.webhook.error`
  - `neuroion.channel`、`neuroion.webhook`、`neuroion.chatId`、`neuroion.error`
- `neuroion.message.processed`
  - `neuroion.channel`、`neuroion.outcome`、`neuroion.chatId`、`neuroion.messageId`、`neuroion.sessionKey`、`neuroion.sessionId`、`neuroion.reason`
- `neuroion.session.stuck`
  - `neuroion.state`、`neuroion.ageMs`、`neuroion.queueDepth`、`neuroion.sessionKey`、`neuroion.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `neuroion doctor`。
