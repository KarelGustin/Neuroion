---
read_when:
  - 设计超越每日 Markdown 日志的工作区记忆（~/.neuroion/workspace）
  - Deciding: standalone CLI vs deep Neuroion integration
  - 添加离线回忆 + 反思（retain/recall/reflect）
summary: 研究笔记：Neuroion 工作区的离线记忆系统（Markdown 作为数据源 + 派生索引）
title: 工作区记忆研究
x-i18n:
...
目标：Neuroion 风格的工作区（`agents.defaults.workspace`，默认 `~/.neuroion/workspace`），其中"记忆"以每天一个 Markdown 文件（`memory/YYYY-MM-DD.md`）加上一小组稳定文件（例如 `memory.md`、`SOUL.md`）的形式存储。
...
保持 `~/.neuroion/workspace` 作为规范的人类可读记忆。
...
- `memory.md` 保持"小型 + 类似核心"：你希望 Neuroion 每次会话都能看到的内容。
