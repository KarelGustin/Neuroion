...
read_when:
  - Setting up Twitch chat integration for Neuroion
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Neuroion connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
neuroion plugins install @neuroion/twitch
```

Local checkout (when running from a git repo):

```bash
neuroion plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `NEUROION_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "neuroion", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use NEUROION_TWITCH_ACCESS_TOKEN env var)
...
```bash
NEUROION_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
