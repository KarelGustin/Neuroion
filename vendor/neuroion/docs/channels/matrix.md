...
Matrix is an open, decentralized messaging protocol. Neuroion connects as a Matrix **user**
...
```bash
neuroion plugins install @neuroion/matrix
```

Local checkout (when running from a git repo):

```bash
neuroion plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `neuroion plugins install @neuroion/matrix`
   - From a local checkout: `neuroion plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Neuroion calls the same
     login endpoint, stores the access token in `~/.neuroion/credentials/matrix/credentials.json`,
...
- On first connection, Neuroion requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Neuroion logs a warning.
...
`~/.neuroion/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `neuroion pairing list matrix`
  - `neuroion pairing approve matrix <CODE>`
...
- On startup, Neuroion resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
