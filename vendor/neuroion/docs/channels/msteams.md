...
```bash
neuroion plugins install @neuroion/msteams
```

Local checkout (when running from a git repo):

```bash
neuroion plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Neuroion will offer the local install path automatically.
...
3. Configure Neuroion with those credentials.
...
- Talk to Neuroion via Teams DMs, group chats, or channels.
...
- On startup, Neuroion resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.neuroion/neuroion.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Neuroion, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `neuroion-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `neuroion plugins install @neuroion/msteams`
   - From a local checkout: `neuroion plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Neuroion in Teams", "full": "Neuroion in Teams" },
...
Neuroion handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Neuroion only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/NeuroionShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Neuroion sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `neuroion message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.neuroion/msteams-polls.json`.
...
```bash
neuroion message send --channel msteams \
...
neuroion message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
neuroion message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
neuroion message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
neuroion message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
