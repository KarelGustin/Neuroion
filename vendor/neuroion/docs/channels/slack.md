...
3. Set tokens for Neuroion and start the gateway.
...
Neuroion can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, Neuroion falls back to the user token.
...
By default, Neuroion replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, Neuroion infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `neuroion pairing approve slack <code>`.
...
- On startup, Neuroion resolves channel/user names in allowlists to IDs (when tokens allow)
