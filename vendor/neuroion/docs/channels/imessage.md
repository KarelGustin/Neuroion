...
3. Configure Neuroion with `channels.imessage.cliPath` and `channels.imessage.dbPath`.
...
- Full Disk Access for Neuroion + `imsg` (Messages DB access).
...
2. Create a macOS user (example: `neuroionhome`) and sign into it.
...
If you want iMessage on another Mac, set `channels.imessage.cliPath` to a wrapper that runs `imsg` on the remote macOS host over SSH. Neuroion only needs stdio.
...
**Remote attachments:** When `cliPath` points to a remote host via SSH, attachment paths in the Messages database reference files on the remote machine. Neuroion can automatically fetch these over SCP by setting `channels.imessage.remoteHost`:
...
If `remoteHost` is not set, Neuroion attempts to auto-detect it by parsing the SSH command in your wrapper script. Explicit configuration is recommended for reliability.
...
│ - neuroion gateway           │          SCP (attachments)        │ - Messages signed in     │
...
```json5
{
  channels: {
    imessage: {
      enabled: true,
      cliPath: "~/.neuroion/scripts/imsg-ssh",
...
Example wrapper (`~/.neuroion/scripts/imsg-ssh`):
...
Multi-account support: use `channels.imessage.accounts` with per-account config and optional `name`. See [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) for the shared pattern. Don't commit `~/.neuroion/neuroion.json` (it often contains tokens).
...
- Approve via:
  - `neuroion pairing list imessage`
  - `neuroion pairing approve imessage <CODE>`
...
If you explicitly configure a `chat_id` under `channels.imessage.groups`, Neuroion treats that thread as a “group” for:
