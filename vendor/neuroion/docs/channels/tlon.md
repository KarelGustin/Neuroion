...
Tlon is a decentralized messenger built on Urbit. Neuroion connects to your Urbit ship and can
...
```bash
neuroion plugins install @neuroion/tlon
```

Local checkout (when running from a git repo):

```bash
neuroion plugins install ./extensions/tlon
```
...
Use these with `neuroion message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Neuroion replies in-thread.
