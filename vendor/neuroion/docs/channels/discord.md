...
- Talk to Neuroion via Discord DMs or guild channels.
...
3. Configure Neuroion with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `neuroion pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running Neuroion in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents Neuroion needs
...
Discord uses numeric ids everywhere; Neuroion config prefers ids.
...
- Owner hint: when a per-guild or per-channel `users` allowlist matches the sender, Neuroion treats that sender as the owner in the system prompt. For a global owner across channels, set `commands.ownerAllowFrom`.
...
- First: run `neuroion doctor` and `neuroion channels status --probe` (actionable warnings + quick audits).
...
When enabled, Neuroion uses the member identity for allowlists and labels the
...
- On startup, Neuroion resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:

- The registered commands mirror Neuroion’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; Neuroion enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`neuroion gateway --force`) after confirming no other processes own the Discord session.
