...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Neuroion to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
neuroion plugins install @neuroion/feishu
```

Local checkout (when running from a git repo):

```bash
neuroion plugins install ./extensions/feishu
```
...
If you just installed Neuroion, run the wizard:
...
2. Configuring app credentials in Neuroion
...
1. You already ran `neuroion channels add` for Feishu
2. The gateway is running (`neuroion gateway status`)
...
```bash
neuroion pairing approve feishu <CODE>
```
...
2. Run `neuroion logs --follow` and look for `chat_id`
...
2. Run `neuroion logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `neuroion gateway status`  | Show gateway status           |
| `neuroion gateway install` | Install/start gateway service |
| `neuroion gateway stop`    | Stop gateway service          |
| `neuroion gateway restart` | Restart gateway service       |
| `neuroion logs --follow`   | Tail gateway logs             |
...
4. Check logs: `neuroion logs --follow`
...
5. Ensure the gateway is running: `neuroion gateway status`
6. Check logs: `neuroion logs --follow`
...
```json5
{
  agents: {
    list: [
      ...
      {
        id: "neuroion-fan",
        workspace: "/home/user/neuroion-fan",
        agentDir: "/home/user/.neuroion/agents/neuroion-fan/agent",
      },
      {
        id: "neuroion-xi",
        workspace: "/home/user/neuroion-xi",
        agentDir: "/home/user/.neuroion/agents/neuroion-xi/agent",
      },
    ],
  },
  bindings: [
