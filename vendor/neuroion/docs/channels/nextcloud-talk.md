...
```bash
neuroion plugins install @neuroion/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
neuroion plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Neuroion" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `neuroion pairing list nextcloud-talk`
  - `neuroion pairing approve nextcloud-talk <CODE>`
