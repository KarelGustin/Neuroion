---
summary: "Use MiniMax M2.1 in Neuroion"
read_when:
  - You want MiniMax models in Neuroion
...
```bash
neuroion plugins enable minimax-portal-auth  # skip if already loaded.
neuroion gateway restart  # restart if gateway is already running
neuroion onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/neuroion/neuroion/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `neuroion.json`:
...
- Use `neuroion models list` and `neuroion models set minimax/MiniMax-M2.1` to switch.
...
- Running `neuroion configure` and selecting **MiniMax M2.1**, or
