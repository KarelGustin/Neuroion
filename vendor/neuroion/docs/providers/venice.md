---
summary: "Use Venice AI privacy-focused models in Neuroion"
read_when:
  - You want privacy-focused inference in Neuroion
...
```bash
neuroion onboard --auth-choice venice-api-key
```
...
```bash
neuroion chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Neuroion shows all available Venice models. Pick based on your needs:
...
```bash
neuroion models set venice/claude-opus-45
neuroion models set venice/llama-3.3-70b
```

List all available models:

```bash
neuroion models list | grep venice
```

## Configure via `neuroion configure`

1. Run `neuroion configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Neuroion automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
neuroion chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
neuroion chat --model venice/claude-opus-45

# Use uncensored model
neuroion chat --model venice/venice-uncensored

# Use vision model with image
neuroion chat --model venice/qwen3-vl-235b-a22b

# Use coding model
neuroion chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `neuroion models list` to see currently available models. Some models may be temporarily offline.
