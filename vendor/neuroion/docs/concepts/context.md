...
read_when:
  - You want to understand what “context” means in Neuroion
...
“Context” is **everything Neuroion sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Neuroion-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Neuroion builds the system prompt

The system prompt is **Neuroion-owned** and rebuilt each run. It includes:
...
By default, Neuroion injects a fixed set of workspace files (if present):
