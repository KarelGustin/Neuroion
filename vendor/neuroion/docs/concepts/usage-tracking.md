...
- `/usage cost` in chats: local cost summary aggregated from Neuroion session logs.
- CLI: `neuroion status --usage` prints a full per-provider breakdown.
- CLI: `neuroion channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
