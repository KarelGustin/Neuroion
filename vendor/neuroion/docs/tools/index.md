---
summary: "Agent tool surface for Neuroion (browser, canvas, nodes, message, cron) replacing legacy `neuroion-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `neuroion-*` skills
title: "Tools"
---

# Tools (Neuroion)

Neuroion exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `neuroion-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `neuroion.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Neuroion logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`neuroion node run`).
...
- Requires a Brave API key (recommended: `neuroion configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Neuroion-managed browser.
...
- Quick smoke: `neuroion nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `neuroion gateway` restart in-place)
