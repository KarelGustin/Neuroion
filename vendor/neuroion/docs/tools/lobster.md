---
title: Lobster
summary: "Typed workflow runtime for Neuroion with resumable approval gates."
description: Typed workflow runtime for Neuroion — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets Neuroion run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Neuroion runs one Lobster tool call and gets a structured result.
...
Neuroion launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | neuroion.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
neuroion.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Neuroion tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Neuroion Gateway (see the [Lobster repo](https://github.com/neuroion/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Neuroion keeps core tools enabled. To restrict core
...
→ neuroion calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ neuroion calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Neuroion tools that do.
