---
summary: "Chrome extension: let Neuroion drive your existing Chrome tab"
read_when:
...
The Neuroion Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate neuroion-managed Chrome profile.
...
Neuroion then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Neuroion release (npm package) as static files. There is no separate “build” step.

After upgrading Neuroion:

- Re-run `neuroion browser extension install` to refresh the installed files under your Neuroion state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Neuroion ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Neuroion to control.
...
- `ON`: attached; Neuroion can drive that tab.
...
Debugging: `neuroion sandbox explain`
...
`neuroion browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `neuroion browser extension install` first to copy the extension to a stable location under your Neuroion state directory.
...
- **This is not isolated** like the dedicated neuroion-managed profile.
