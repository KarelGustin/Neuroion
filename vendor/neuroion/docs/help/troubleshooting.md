...
You can also set `NEUROION_VERBOSE=1` instead of the flag.
...
### `docs.neuroion.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.neuroion.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.neuroion.ai` to the allowlist, then retry.
...
- Check the allowlist: `neuroion config get agents.defaults.models`
...
If you can, include the relevant log tail from `neuroion logs --follow`.
