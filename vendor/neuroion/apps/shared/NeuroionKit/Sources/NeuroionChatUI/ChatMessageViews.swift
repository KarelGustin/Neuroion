...
struct ChatMessageBubble: View {
    let message: NeuroionChatMessage
    let style: NeuroionChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? NeuroionChatTheme.userText : NeuroionChatTheme.assistantText
    ...
    private var inlineAttachments: [NeuroionChatMessageContent] {
    ...
    private var toolCalls: [NeuroionChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [NeuroionChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? NeuroionChatTheme.userBubble
        }
        if self.style == .onboarding {
            return NeuroionChatTheme.onboardingAssistantBubble
        }
        return NeuroionChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return NeuroionChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: NeuroionChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? NeuroionChatTheme.userText : NeuroionChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: NeuroionChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(NeuroionChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(NeuroionChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [NeuroionChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: NeuroionChatTheme.assistantText)
            }
        }
    }
}
