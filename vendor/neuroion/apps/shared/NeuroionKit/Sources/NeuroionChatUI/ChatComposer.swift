...
@MainActor
struct NeuroionChatComposer: View {
    @Bindable var viewModel: NeuroionChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(NeuroionChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(NeuroionChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \NeuroionPendingAttachment.id)
                { (att: NeuroionPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            NeuroionPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(NeuroionChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(NeuroionChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(NeuroionChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(NeuroionChatTheme.subtleCard)
        .clipShape(Capsule())
    }
