...
private let chatUILogger = Logger(subsystem: "ai.neuroion", category: "NeuroionChatUI")

@MainActor
@Observable
public final class NeuroionChatViewModel {
    public private(set) var messages: [NeuroionChatMessage] = []
    ...
    public var attachments: [NeuroionPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [NeuroionChatPendingToolCall] = []
    public private(set) var sessions: [NeuroionChatSessionEntry] = []
    private let transport: any NeuroionChatTransport
    ...
    private var pendingToolCallsById: [String: NeuroionChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any NeuroionChatTransport) {
    ...
    public var sessionChoices: [NeuroionChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [NeuroionChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [NeuroionChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: NeuroionPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [NeuroionChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: NeuroionChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [NeuroionChatMessage]) -> [NeuroionChatMessage] {
        var result: [NeuroionChatMessage] = []
    ...
    private static func dedupeKey(for message: NeuroionChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [NeuroionChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> NeuroionChatAttachmentPayload in
            NeuroionChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> NeuroionChatSessionEntry {
        NeuroionChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: NeuroionChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: NeuroionChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: NeuroionAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = NeuroionChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> NeuroionPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
