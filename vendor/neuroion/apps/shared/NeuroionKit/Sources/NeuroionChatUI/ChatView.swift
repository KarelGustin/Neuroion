...
@MainActor
public struct NeuroionChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: NeuroionChatViewModel
    ...
    public init(
        viewModel: NeuroionChatViewModel,
    ...
    private var visibleMessages: [NeuroionChatMessage] {
        let base: [NeuroionChatMessage]
    ...
    private func mergeToolResults(in messages: [NeuroionChatMessage]) -> [NeuroionChatMessage] {
        ...
        for message in messages {
            ...
            let merged = NeuroionChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: NeuroionChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func toolCallIds(in message: NeuroionChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: NeuroionChatMessage) -> String {
