...
import Foundation
import Testing
@testable import NeuroionChatUI
...
private final class TestChatTransport: @unchecked Sendable, NeuroionChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [NeuroionChatHistoryPayload]
    private let sessionsResponses: [NeuroionChatSessionsListResponse]

    private let stream: AsyncStream<NeuroionChatTransportEvent>
    private let continuation: AsyncStream<NeuroionChatTransportEvent>.Continuation

    init(
        historyResponses: [NeuroionChatHistoryPayload],
        sessionsResponses: [NeuroionChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<NeuroionChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> NeuroionChatHistoryPayload {
        ...
        return self.historyResponses.last ?? NeuroionChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [NeuroionChatAttachmentPayload]) async throws -> NeuroionChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return NeuroionChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> NeuroionChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? NeuroionChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: NeuroionChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = NeuroionChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = NeuroionChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { NeuroionChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = NeuroionChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = NeuroionChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = NeuroionChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { NeuroionChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
