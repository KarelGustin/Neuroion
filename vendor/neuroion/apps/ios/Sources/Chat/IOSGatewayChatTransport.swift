...
struct IOSGatewayChatTransport: NeuroionChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> NeuroionChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(NeuroionChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> NeuroionChatHistoryPayload {
        ...
        return try JSONDecoder().decode(NeuroionChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [NeuroionChatAttachmentPayload]) async throws -> NeuroionChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [NeuroionChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(NeuroionChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(NeuroionGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: NeuroionGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: NeuroionChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: NeuroionAgentEventPayload.self)
                        {
