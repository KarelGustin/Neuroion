...
actor CameraController {
    ...
    func snap(params: NeuroionCameraSnapParams) async throws -> (
        format: String,
        base64: String,
        width: Int,
        height: Int)
    ...
    func clip(params: NeuroionCameraClipParams) async throws -> (
        format: String,
        base64: String,
        durationMs: Int,
        hasAudio: Bool)
    {
        ...
        let movURL = FileManager().temporaryDirectory
            .appendingPathComponent("neuroion-camera-\(UUID().uuidString).mov")
        let mp4URL = FileManager().temporaryDirectory
            .appendingPathComponent("neuroion-camera-\(UUID().uuidString).mp4")
