...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [NeuroionCapability.canvas.rawValue, NeuroionCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(NeuroionCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(NeuroionCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = NeuroionLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(NeuroionCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            NeuroionSystemCommand.notify.rawValue,
            NeuroionSystemCommand.which.rawValue,
            NeuroionSystemCommand.run.rawValue,
            NeuroionSystemCommand.execApprovalsGet.rawValue,
            NeuroionSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(NeuroionCapability.camera.rawValue) {
            commands.append(NeuroionCameraCommand.list.rawValue)
            commands.append(NeuroionCameraCommand.snap.rawValue)
            commands.append(NeuroionCameraCommand.clip.rawValue)
        }
        if caps.contains(NeuroionCapability.location.rawValue) {
            commands.append(NeuroionLocationCommand.get.rawValue)
        }
