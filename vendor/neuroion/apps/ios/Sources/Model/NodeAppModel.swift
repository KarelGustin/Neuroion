...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = NeuroionCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = NeuroionCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = NeuroionCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = NeuroionCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = NeuroionCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__neuroion__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: NeuroionLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for neuroion:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case NeuroionLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case NeuroionCanvasCommand.present.rawValue,
                 NeuroionCanvasCommand.hide.rawValue,
                 NeuroionCanvasCommand.navigate.rawValue,
                 NeuroionCanvasCommand.evalJS.rawValue,
                 NeuroionCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case NeuroionCanvasA2UICommand.reset.rawValue,
                 NeuroionCanvasA2UICommand.push.rawValue,
                 NeuroionCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case NeuroionCameraCommand.list.rawValue,
                 NeuroionCameraCommand.snap.rawValue,
                 NeuroionCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case NeuroionScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: NeuroionNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: NeuroionNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(NeuroionLocationGetParams.self, from: req.paramsJSON)) ??
            NeuroionLocationGetParams()
        ...
        let payload = NeuroionLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case NeuroionCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(NeuroionCanvasPresentParams.self, from: req.paramsJSON)) ??
                NeuroionCanvasPresentParams()
        ...
        case NeuroionCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case NeuroionCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(NeuroionCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case NeuroionCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(NeuroionCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case NeuroionCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(NeuroionCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case NeuroionCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              ...
              if (!host) return JSON.stringify({ ok: false, error: "missing neuroionA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case NeuroionCanvasA2UICommand.push.rawValue, NeuroionCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == NeuroionCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(NeuroionCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try NeuroionCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(NeuroionCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try NeuroionCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.neuroionA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case NeuroionCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case NeuroionCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(NeuroionCameraSnapParams.self, from: req.paramsJSON)) ??
                NeuroionCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case NeuroionCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(NeuroionCameraClipParams.self, from: req.paramsJSON)) ??
                NeuroionCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(NeuroionScreenRecordParams.self, from: req.paramsJSON)) ??
            NeuroionScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> NeuroionLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return NeuroionLocationMode(rawValue: raw) ?? .off
    }
