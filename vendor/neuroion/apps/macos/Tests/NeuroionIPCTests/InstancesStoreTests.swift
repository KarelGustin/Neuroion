...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: NeuroionProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: NeuroionProtocol.AnyCodable] = [
            "presence": .init([NeuroionProtocol.AnyCodable(entry)]),
        ]
        let payload = NeuroionProtocol.AnyCodable(payloadMap)
