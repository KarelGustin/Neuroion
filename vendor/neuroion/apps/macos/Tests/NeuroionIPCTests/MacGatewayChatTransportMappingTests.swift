...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: NeuroionProtocol.AnyCodable(["ok": NeuroionProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: NeuroionProtocol.AnyCodable(["ok": NeuroionProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = NeuroionProtocol.AnyCodable([
            "runId": NeuroionProtocol.AnyCodable("run-1"),
            "sessionKey": NeuroionProtocol.AnyCodable("main"),
            "state": NeuroionProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: NeuroionProtocol.AnyCodable(["a": NeuroionProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
