...
struct ConnectOptions {
    ...
    var clientId: String = "neuroion-macos"
...
func runConnect(_ args: [String]) async {
    ...
    if opts.help {
        print("""
        neuroion-mac connect

        Usage:
          neuroion-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
        ...
        Options:
          ...
          --json             Emit JSON
          --client-id <id>   Override client id (default: neuroion-macos)
    ...
    do {
        ...
        let displayName = opts.displayName ?? Host.current().localizedName ?? "Neuroion macOS Debug CLI"
...
private func printConnectOutput(_ output: ConnectOutput, json: Bool) {
    ...
    print("Neuroion macOS Gateway Connect")
