...
func runWizardCommand(_ args: [String]) async {
    ...
    if opts.help {
        ...
        Usage:
          neuroion-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
...
actor GatewayWizardClient {
    ...
    private func sendConnect() async throws {
        ...
        let clientId = "neuroion-macos"
        let clientMode = "ui"
        let role = "operator"
        let scopes: [String] = []
        let client: [String: ProtoAnyCodable] = [
            "id": ProtoAnyCodable(clientId),
            "displayName": ProtoAnyCodable(Host.current().localizedName ?? "Neuroion macOS Wizard CLI"),
