...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.neuroion", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: NeuroionPeekabooBridgeServices?
    private static var neuroionSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Neuroion", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = NeuroionPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.neuroionSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.neuroionSocketPath, privacy: .public)")
    }
...
@MainActor
private final class NeuroionPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.neuroion.peekaboo")
