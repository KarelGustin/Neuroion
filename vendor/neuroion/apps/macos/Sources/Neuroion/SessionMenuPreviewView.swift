...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.neuroion", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> NeuroionSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: NeuroionSessionPreviewEntry,
    ...
    private static func cache(payload: NeuroionSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: NeuroionChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [NeuroionKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [NeuroionKit.AnyCodable]) -> [NeuroionChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(NeuroionChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: NeuroionChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: NeuroionChatMessage) -> Bool {
    ...
    private static func toolNames(for message: NeuroionChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: NeuroionChatMessage) -> String? {
