...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.neuroion/neuroion.json. Prefer NEUROION_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.neuroion/neuroion.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.neuroion/neuroion.json. Restart the gateway to apply."
        }
