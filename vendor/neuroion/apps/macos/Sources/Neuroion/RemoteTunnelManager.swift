...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.neuroion", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Neuroion run already has an SSH listener on the expected port (common after restarts),
