...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Neuroion token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: NeuroionOAuthStore.AnthropicOAuthStatus = NeuroionOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.neuroion", category: "anthropic-oauth")
...
enum NeuroionOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let neuroionOAuthDirEnv = "NEUROION_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Neuroion OAuth token file not found"
            case .unreadableFile: "Neuroion OAuth token file not readable"
            case .invalidJSON: "Neuroion OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Neuroion OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Neuroion OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.neuroionOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".neuroion", isDirectory: true)
