        ...
        ScrollView(.vertical) {
            VStack(alignment: .leading, spacing: 18) {
                VStack(alignment: .leading, spacing: 12) {
                    SettingsToggleRow(
                        title: "Neuroion active",
                        subtitle: "Pause to stop the Neuroion gateway; no messages will be processed.",
                    ...
                    SettingsToggleRow(
                        ...
                        subtitle: "Automatically start Neuroion after you sign in.",
                        binding: self.$state.launchAtLogin)

                    SettingsToggleRow(
                        title: "Show Dock icon",
                        subtitle: "Keep Neuroion visible in the Dock instead of menu-bar-only mode.",
                ...
                HStack {
                    Spacer()
                    Button("Quit Neuroion") { NSApp.terminate(nil) }
                ...
                DisclosureGroup(isExpanded: self.$showRemoteAdvanced) {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        LabeledContent("CLI path") {
                            TextField("/Applications/Neuroion.app/.../neuroion", text: self.$state.remoteCliPath)
                                .textFieldStyle(.roundedBorder)
                                .frame(width: 280)
                        }
                    }
...
extension GeneralSettings {
    ...
    private func revealLogs() {
        ...
        Looked for neuroion logs in /tmp/neuroion/.
    ...
    private func applyDiscoveredGateway(_ gateway: GatewayDiscoveryModel.DiscoveredGateway) {
        ...
        if self.state.remoteTransport == .direct {
            ...
            NeuroionConfigFile.setRemoteGatewayUrl(host: host, port: gateway.gatewayPort)
        }
    }
}
...
extension GeneralSettings {
    static func exerciseForTesting() {
        ...
        state.remoteProjectRoot = "/tmp/neuroion"
        state.remoteCliPath = "/tmp/neuroion"
