...
private let webChatSwiftLogger = Logger(subsystem: "ai.neuroion", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: NeuroionChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> NeuroionChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(NeuroionChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> NeuroionChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                NeuroionGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    NeuroionAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<NeuroionChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any NeuroionChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = NeuroionChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: NeuroionChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<NeuroionChatView>) -> NSViewController
    {
