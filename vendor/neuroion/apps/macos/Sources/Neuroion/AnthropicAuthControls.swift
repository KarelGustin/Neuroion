...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: NeuroionOAuthStore.AnthropicOAuthStatus = NeuroionOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: NeuroionOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = NeuroionOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = NeuroionOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try NeuroionOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Neuroion can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: NeuroionOAuthStore.AnthropicOAuthStatus,
