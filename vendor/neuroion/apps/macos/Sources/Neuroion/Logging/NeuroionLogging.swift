...
enum NeuroionLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = NeuroionOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = NeuroionFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        NeuroionLogging.bootstrapIfNeeded()
        let label = NeuroionLogging.makeLabel(subsystem: subsystem, category: category)
...
struct NeuroionOSLogHandler: LogHandler {
...
struct NeuroionFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = NeuroionLogging.parseLabel(self.label)
