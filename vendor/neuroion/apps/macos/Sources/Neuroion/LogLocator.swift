...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["NEUROION_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/neuroion")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("neuroion-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("neuroion-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/neuroion/ (rolling or stdout), or nil if none exist.
