...
struct PermissionsSettings: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("Allow these so Neuroion can notify and capture when needed.")
                .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = NeuroionLocationMode.off.rawValue
    @AppStorage(locationPreciseKey) private var locationPreciseEnabled: Bool = true
    @State private var lastLocationModeRaw: String = NeuroionLocationMode.off.rawValue
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Picker("", selection: self.$locationModeRaw) {
                Text("Off").tag(NeuroionLocationMode.off.rawValue)
                Text("While Using").tag(NeuroionLocationMode.whileUsing.rawValue)
                Text("Always").tag(NeuroionLocationMode.always.rawValue)
            }
            .labelsHidden()
            .pickerStyle(.menu)
        ...
        .onChange(of: self.locationModeRaw) { _, newValue in
            ...
            guard let mode = NeuroionLocationMode(rawValue: newValue) else { return }
            Task {
    ...
    private var locationMode: NeuroionLocationMode {
        NeuroionLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: NeuroionLocationMode) async -> Bool {
