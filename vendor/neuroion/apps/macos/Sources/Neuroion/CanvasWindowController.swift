...
final class CanvasWindowController: NSWindowController, WKNavigationDelegate, NSWindowDelegate {
    ...
    init(sessionKey: String, root: URL, presentation: CanvasPresentation) throws {
        ...
        (() => {
          try {
            ...
            if (globalThis.__neuroionA2UIBridgeInstalled) return;
            globalThis.__neuroionA2UIBridgeInstalled = true;
            ...
            globalThis.addEventListener('a2uiaction', (evt) => {
              try {
                ...
                const handler = globalThis.webkit?.messageHandlers?.neuroionCanvasA2UIAction;
                ...
                const hasBundledA2UIHost =
                  !!globalThis.neuroionA2UI ||
                  !!document.querySelector('neuroion-a2ui-host');
                ...
                location.href = 'neuroion://agent?' + params.toString();
              } catch {}
            }, true);
          } catch {}
        })();
    ...
    func applyDebugStatusIfNeeded() {
        ...
        (() => {
          try {
            const api = globalThis.__neuroion;
            if (!api) return;
    ...
    func snapshot(to outPath: String?) async throws -> String {
        ...
        if let outPath, !outPath.isEmpty {
            ...
            path = "/tmp/neuroion-canvas-\(CanvasWindowController.sanitizeSessionKey(self.sessionKey))-\(ts).png"
        }
