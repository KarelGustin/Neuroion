...
final class WorkActivityStore {
    ...
    func handleTool(
        ...
        name: String?,
        meta: String?,
        args: [String: NeuroionProtocol.AnyCodable]?)
    {
    ...
    private static func buildLabel(
        name: String?,
        meta: String?,
        args: [String: NeuroionProtocol.AnyCodable]?) -> String
    {
    ...
    private static func wrapToolArgs(_ args: [String: NeuroionProtocol.AnyCodable]?) -> NeuroionKit.AnyCodable? {
        guard let args else { return nil }
        let converted: [String: Any] = args.mapValues { self.unwrapJSONValue($0.value) }
        return NeuroionKit.AnyCodable(converted)
    }

    private static func unwrapJSONValue(_ value: Any) -> Any {
        if let dict = value as? [String: NeuroionProtocol.AnyCodable] {
            return dict.mapValues { self.unwrapJSONValue($0.value) }
        }
        if let array = value as? [NeuroionProtocol.AnyCodable] {
