...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: NeuroionProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.neuroion", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: NeuroionKit.AnyCodable]()) {
                $0[$1.key] = NeuroionKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: NeuroionProtocol.AnyCodable?) -> [String: NeuroionProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: NeuroionProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: NeuroionKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: NeuroionProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("neuroion.control.heartbeat")
    static let controlAgentEvent = Notification.Name("neuroion.control.agent")
}
