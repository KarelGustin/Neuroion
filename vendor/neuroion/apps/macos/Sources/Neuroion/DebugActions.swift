...
enum DebugActions {
    private static let verboseDefaultsKey = "neuroion.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "neuroion.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = NeuroionPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Neuroion", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = NeuroionPaths.configURL
        guard
