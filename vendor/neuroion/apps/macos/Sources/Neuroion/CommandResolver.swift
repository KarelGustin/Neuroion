...
enum CommandResolver {
    private static let projectRootDefaultsKey = "neuroion.gatewayProjectRootPath"
    private static let helperName = "neuroion"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let neuroionEntry = root.appendingPathComponent("neuroion.mjs").path
        if FileManager().isReadableFile(atPath: neuroionEntry) { return neuroionEntry }
        let binEntry = root.appendingPathComponent("bin/neuroion.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/neuroion")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let neuroionPaths = self.neuroionManagedPaths(home: home)
        if !neuroionPaths.isEmpty {
            extras.insert(contentsOf: neuroionPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + neuroionPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func neuroionManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".neuroion"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func neuroionExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectNeuroionExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("neuroion.mjs").path,
            root.appendingPathComponent("bin/neuroion.js").path,
        ]
    ...
    static func hasAnyNeuroionInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.neuroionExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func neuroionNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let neuroionPath = self.projectNeuroionExecutable(projectRoot: root) {
                return [neuroionPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "neuroion", subcommand] + extraArgs
            }
            if let neuroionPath = self.neuroionExecutable(searchPaths: searchPaths) {
                return [neuroionPath, subcommand] + extraArgs
            }

            let missingEntry = """
            neuroion entrypoint missing (looked for dist/index.js or neuroion.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real neuroion CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/neuroion"
        ...
        \(cliSection)
        \(projectSection)
        if command -v neuroion >/dev/null 2>&1; then
          CLI="$(command -v neuroion)"
          neuroion \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/neuroion.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/neuroion.mjs"
            node "$PRJ/neuroion.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/neuroion.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/neuroion.js"
            node "$PRJ/bin/neuroion.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent neuroion"
          pnpm --silent neuroion \(quotedArgs);
        else
          echo "neuroion CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? NeuroionConfigFile.loadDict()
