...
enum WideAreaGatewayDiscovery {
        ...
        context: DiscoveryContext = .live) -> [WideAreaGatewayBeacon]
    {
        ...
        let probeName = "_neuroion-gw._tcp.\(domainTrimmed)"
        ...
        for raw in ptrLines {
            ...
            let suffix = "._neuroion-gw._tcp.\(domainTrimmed)"
    ...
    {
        guard let domain = NeuroionBonjour.wideAreaGatewayServiceDomain else { return nil }
        let domainTrimmed = domain.trimmingCharacters(in: CharacterSet(charactersIn: "."))
        let probeName = "_neuroion-gw._tcp.\(domainTrimmed)"
