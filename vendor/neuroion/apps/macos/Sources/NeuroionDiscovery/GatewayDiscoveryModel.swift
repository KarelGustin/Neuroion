...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.neuroion", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in NeuroionBonjour.gatewayServiceDomains {
            let params = NWParameters.tcp
            params.includePeerToPeer = true
            let browser = NWBrowser(
                for: .bonjour(type: NeuroionBonjour.gatewayServiceType, domain: domain),
                using: params)
            ...
            browser.start(queue: DispatchQueue(label: "ai.neuroion.macos.gateway-discovery.\(domain)"))
        }
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        guard let domain = NeuroionBonjour.wideAreaGatewayServiceDomain else { return }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = NeuroionBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = NeuroionBonjour.wideAreaGatewayServiceDomain else { return false }
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (Neuroion)", with: "")
