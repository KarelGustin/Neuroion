...
public struct CanvasShowResult: Codable, Sendable {
    /// Session directory on disk (e.g. `~/Library/Application Support/Neuroion/canvas/<session>/`).
...
public let controlSocketPath: String = {
    ...
    let preferred = home
        .appendingPathComponent("Library/Application Support/Neuroion/control.sock")
        .path
    return preferred
}()
