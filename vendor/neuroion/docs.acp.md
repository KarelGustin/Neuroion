...
This document describes how the Neuroion ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`neuroion acp` exposes an ACP agent over stdio and forwards prompts to a running
Neuroion Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Neuroion Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `neuroion acp` over stdio.

Example config:

```bash
neuroion config set gateway.remote.url wss://gateway-host:18789
neuroion config set gateway.remote.token <token>
```

Example run:

```bash
neuroion acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
neuroion acp --session agent:main:main
neuroion acp --session agent:design:main
neuroion acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Neuroion ACP” to start a thread.

## Execution Model

- ACP client spawns `neuroion acp` and speaks ACP messages over stdio.
...
```bash
neuroion acp --session agent:main:main
neuroion acp --session-label "support inbox"
neuroion acp --reset-session
```
...
`neuroion acp` resolves the Gateway URL and auth from CLI flags or config:
