...
import type { NeuroionConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("neuroion-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("neuroion-bin-sherpa-");
      const modelDir = await makeTempDir("neuroion-sherpa-model-");
      ...
      const cfg: NeuroionConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("neuroion-bin-whispercpp-");
      const modelDir = await makeTempDir("neuroion-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("neuroion-bin-gemini-");
      ...
      const cfg: NeuroionConfig = { tools: { media: { image: {} } } };
