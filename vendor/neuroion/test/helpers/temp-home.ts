...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("NEUROION_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.NEUROION_STATE_DIR = path.join(base, ".neuroion");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "neuroion-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".neuroion", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
