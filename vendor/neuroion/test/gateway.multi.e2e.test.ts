...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `neuroion-e2e-${name}-`));
  const configDir = path.join(homeDir, ".neuroion");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          NEUROION_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        NEUROION_GATEWAY_TOKEN: inst.gatewayToken,
        NEUROION_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          NEUROION_GATEWAY_PORT: String(gwA.port),
          NEUROION_GATEWAY_TOKEN: gwA.gatewayToken,
          NEUROION_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          NEUROION_GATEWAY_PORT: String(gwB.port),
          NEUROION_GATEWAY_TOKEN: gwB.gatewayToken,
          NEUROION_GATEWAY_PASSWORD: "",
        }),
